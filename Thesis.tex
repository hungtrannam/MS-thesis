...
Hàm mật độ xác suất là khách thể nghiên cứu của các nhà thống kê từ những năm 2010 bởi Tai Vo-Van \cite{Tai2010}. Nó là một phần của dữ liệu đặc trưng\index{dữ liệu đặc trưng (symbolic data)} bao gồm hữu hạn các giá trị, khoảng và hàm mật độ. Ở đây, luận văn chỉ hệ thống hóa những kiến thức có liên quan.
...
\dng{Với các đối tượng hàm mật độ, toán tử tổng và tích có hướng được thay thế bởi tích chập và lũy thừa. Tức là, $(f\oplus g)(x)=f(x)g(x)$ và $(\alpha\odot f)(x) = f^{\alpha}(x)$, với $\alpha\in\mathbb{R}$.}
...
\dng{[Hàm mật độ đại diện Gauss] Hàm mật độ đại diện Gauss có dạng phân phối chuẩn sao cho tổng khoảng cách giữa chúng đến các hàm mật độ khác là nhỏ nhất, tức là
$$\theta_i(x) = \arg\min_{\mathcal{N}(\mu\,,\sigma)}\sum_{j=1}^n\mu_{ij}D(\mathcal{N}(\mu\,,\sigma)\,,f_j(x)).$$
}

	 Hàm mật độ đại diện Gauss là hàm trung bình Fréchet nên có nghiệm duy nhất. Khác với \eqref{daidien}, hàm mật độ này chỉ cần tìm một hàm Gauss duy nhất mà không cần tổ hợp các $f_j$ giúp tăng khả năng khái quát hóa. 
...
\begin{table}[H]
	...
	\begin{tabular*}{\columnwidth}{@{\extracolsep\fill}lllccc@{\extracolsep\fill}} 
		...
		$\mathcal{L}_p$ & $\left(\int_{\Omega}|f(x) - g(x)|^p\mathrm{\,d}x\right)^{1/p}$ & -- & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
...
\mathbb{P}(c_k \,|\, h) = \frac{\mathbb{P}(c_k) f(h \,|\, c_k)}{\sum_i \mathbb{P}(c_i) f(h\,|\, c_i)},$$ và phân loại $h$ vào lớp $c_k$ nào có xác suất hậu nghiệm lớn nhất, $c_i  = \arg\max_{i}\mathbb{P}(c_i \,|\, h)$.
...
\subsection{Lịch sử nghiên cứu}


\subsubsection{a.\qquad Bài toán phân tích chùm cho hàm mật độ xác suất}
...
\begin{table}[H]
	...
	\begin{tabular}{lllll}
		\toprule
		\textbf{Năm} & \textbf{Loại mô hình} & \textbf{Phương pháp} &\textbf{Khoảng cách} & \textbf{Tự động số cụm} \\
		\midrule
	\cite{vovan2019cluster}	2019 & Cứng & k-Medoids &  CWD-based & Không \\
		
		2019 & Cứng & Binary elitist DE &  $\mathcal{L}_2$ & Có \\
		
		2020 & Mờ & Genetic Alg. &  $\mathcal{L}_1$ & Có \\
		
		2020 & Entropy & Jackknife entropy &$\mathcal{L}_1$ Kernel Gauss & Không \\
		
		2021 & Mờ& Kernel K-Medoids & $\mathcal{L}_1$, Kernel Gauss & Không \\
		
	\cite{nguyen2023balance}	2022 & Mờ & Balance-driven + MO &  $\mathcal{L}_2$  & Có \\
		
		2023 & Mờ & Prototype-based DE &$\mathcal{L}_1$  Kernel Gauss & Có \\
		
	\cite{nguyen2023globally}	2023 &Mờ & GA cải tiến &  $\mathcal{L}_1$ & Có \\
		
		2024 & Khả dĩ & Possibilistic objective & $\mathcal{L}_2$ & Không \\
		
		2024 & Mờ bán giám sát & Fuzzy + bán giám sát & $\mathcal{L}_1$ & Có \\
		\bottomrule
	\end{tabular}
	\caption{Tổng quan lịch sử nghiên cứu phân tích chùm cho hàm mật độ xác suất}
	\label{tab:pdf_clustering_summary}
\end{table}
...
\subsubsection{b.\qquad Bài toán phân loại cho hàm mật độ xác suất}
...
$ARI = \frac{RI - \mathbb{E}(RI)}{\max(RI) - \mathbb{E}(RI)},$
	...
	\item[Fowlkes–Mallows Index] Đo độ tương đồng cặp đúng/sai
	$
	FMI = \sqrt{\frac{a}{a+b} \cdot \frac{a}{a+c}}
	$
		\item[Jaccard Index] Tỷ lệ cặp cùng một chùm so với tổng cặp liên quan
	$
	J = \frac{a}{a + b + c}
	$
