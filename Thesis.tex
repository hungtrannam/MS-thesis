...
\begin{table}[H]
	...
	\begin{tabular*}{\columnwidth}{@{\extracolsep\fill}lllll@{\extracolsep\fill}} 
		...
		bình phương Hellinger & $\frac{1}{2}\displaystyle\int_{\Omega}\left(\sqrt{f_1(x)}-\sqrt{f_2(x)}\right)^2\mathrm{\,d}x$ & $(\sqrt{u}-1)^2$ & $\frac{1}{2}x$ & $[0\,;1]$ \\
		thông tin Kullback&$\displaystyle\int_{\Omega} f_1(x)\log\frac{f_1(x)}{f_2(x)}\mathrm{\,d}x$ & $-\log(u)$ & $u$ & $[0\,;+\infty)$ \\
		 Jensen-Shannon & $\frac{1}{2}\left(D_{KL}(f_1\,, f_2) + D_{KL}(f_2\,, f_1)\right)$ & $(u-1)\log(u)$ & $\frac{1}{2}u$ & $[0\,;+\infty)$ \\
...
\chy{Nhìn vào Bảng \ref{tab1:KC_PDF}, có thể thấy khoảng cách Chernoff là một tổng quát hóa của khoảng cách Bhattacharyya khi chọn $s = 1/2$. Trong $\mathcal{L}_p$, chỉ có $\mathcal{L}_1$ là một $f$-phân kỳ và là Matusita với $r=1$. Đáng chú ý, ngoài $\mathcal{L}_1$, cả hai loại khoảng cách Hellinger và Matusita đều thỏa mãn tính chất mê-tríc.
}

Ở Hình~\ref{fig:dayKC}, chúng tôi ghi nhận giá trị khoảng cách $f_1\sim\mathcal{N}(0\,,1)$ và $f_2\sim \mathcal{N}(\mu\,,1)$ với $\mu$ trượt từ $-10$ đến $20$.
...
\begin{table}[H]
	...
	\begin{tabular*}{\textwidth}{@{\extracolsep\fill}lp{4cm}p{6cm}lp{1.5cm}@{\extracolsep\fill}} 
		...
		2021 & Tavakkol, Behnam \& Son, Youngdoo \cite{tavakkol2021fuzzy} & Phân chùm mờ kernel $k$-medoids cho dữ liệu hàm mật độ $2$-chiều, $m=2$, số chùm cho trước& hạt nhân Gauss $\mathcal{L}_1$& $\times$   \\
		\bottomrule
	\end{tabular*}
	\begin{flushleft}\it\footnotesize
...
Hàm mục tiêu của EM-IFCF là tổng của hai phần bao gồm trọng số $1/\omega_j$ giảm ``hiệu ứng đồng nhất'' và hiệu chỉnh biên \index{hiệu chỉnh biên (edge modification)} giảm ``hiệu ứng xi-phông''.
\begin{align}
	\tag{EM-IFCF}\label{eq:EMIFCF}
\text{Minimise}\,&:\, J(\bm U\,,\Theta\,|\, \mathcal{F}\,,c\,,m\,,\delta') = 
	\underbrace{\sum_{j=1}^n \sum_{i=1}^c \frac{1}{\omega_j}\, \mu_{ij}^m \,D^2(f_j\,,\theta_i)}_{IFCF}
	\;+\;
...
\end{align}
trong đó $\omega_i=\frac{1}{n}\sum_{j=1}^n \mu^{(t-1)}_{ij}$ là tỷ lệ kích thước mờ của chùm thứ $i$ thỏa mãn $\omega_1+\cdots+\omega_c=1$ với $\mu^{(t-1)}_{ij}$ là ma trận phân vùng tại vòng lặp thứ $(t-1)$, và $D^*_{ij}$ là chuẩn hóa  $[0\,,1]$ của ma trận khoảng cách ở vòng lặp trước được tính bởi
\begin{align}
	D^*_{ij}\;=\;\frac{D^2\left (\theta_i^{(t-1)}\,,f_j\right )}{\sum_{s=1}^c D^2\left (\theta_s^{(t-1)}\,,f_j\right)},	\label{eq:Dstar}
\end{align}
và $\delta_i$ là hệ số điều khiển. 
Khi $\omega_j= 1$ và $\delta'=0$ thì thuật toán trở thành FCF. 
...
$. Nó tiến gần $0$ với biên và lớn hơn khi không phải. Đồng thời, nhân tử $D^{*}_{ij}$ cung cấp hướng phạt làm triệt tiêu xu hướng hút của hiệu ứng.
...
Trong phần này, một dãy ${\left (\bm\mu^{(t)}\,, \Theta^{(l)} \right )}$ khởi tạo được thành lập. Mục tiêu là chứng minh dãy này có hội tụ hay không \cite{bezdek1980convergence,yu2024suppressed}.  Cho
 $
F\,:\,\mathbb{R}^{cd} \to \bm{U}_f\,, F(\Theta) = \bm{\mu}\,, \text{và }
G\,:\,\bm{U}_f \to \mathbb{R}^{cd} \,, G(\bm{\mu})=\Theta$, trong đó, $\Theta = (\theta_1\,,\theta_2\,, \ldots\,,\theta_c)$ và các hàm mật độ trọng tâm $\theta_i$ tính bởi công thức \eqref{trongtamFCF}. Ta định nghĩa toán tử $T_{EM-IFCF}\,:\,\mathbb{R}^{cd} \times \bm{U}_f\to \mathbb{R}^{cd} \times \bm{U}_f\,, T_{ EM-IFCF} = T_2\circ T_1,$ trong đó ánh xạ  $T_1\,:\,\mathbb{R}^{cd} \times \bm{U}_f \to \mathbb{R}^{cd} \,, T_1(\bm{\mu} \,,\Theta)=G(\bm \mu)$, và $T_2\,:\, \mathbb{R}^{cd} \to \mathbb{R}^{cd}\times\bm{U}_f \,, T_2(\Theta)= (F(\Theta), \Theta)$. Khi này, ta có $T_{ EM-IFCF} = (T_2\circ T_1)((\bm \mu\,,\Theta) = \left(F\circ (\bm\mu)\,,G(\bm\mu)\right)$. Tức là
 \begin{align}\label{TO}
	T_{EM-IFCF}\,:\,\mathbb{R}^{cd} \times \bm{U}_f\to \mathbb{R}^{cd} \times \bm{U}_f\,, T_{EM-IFCF} = T_2\circ T_1,
\end{align}
...
\bode{[Cập nhật ma trận phân vùng $\bm U$]\label{lem1}
	Trong EM-IFCF, giá trị phân vùng $\mu_{ij}$ của hàm mật độ $f_j$ đối với chùm $i$ được xác định bởi công thức
	\begin{align}	\label{eq:emifcf_u}
		\mu_{ij} =
		\frac{
			\left[\tfrac{(1 - \delta' D^*_{ij}) \omega_j}{D^2(\theta_i\,,f_j)- f_j \delta_i D^*_{ij}}\right]^{\frac{1}{m-1}}
		}{
			\sum\limits_{s=1}^c 
			\left[\tfrac{(1 - \delta' D^*_{is}) \omega_s}{D^2(\theta_s\,,f_j) - f_s \delta_i D^*_{is}}\right]^{\frac{1}{m-1}}
		}.	
	\end{align}
}

\chm{
	Đưa ràng buộc \eqref{c1_pro} vào hàm Lagrange ta nhận được bài toán tối ưu không ràng buộc
	\begin{align}\label{eq:Lagrange1}
		J(\bm U,\Theta,\Lambda) =\underbrace{ J(\bm U\,,\Theta) }_{\text{EM-IFCF}}-\underbrace{\sum_{i=1}^n \lambda_i\Bigl(\sum_{j=1}^c u_{ij}-1\Bigr)}_{\text{nhân tử Lagrange}}
	\end{align}
	
	Đối với ma trận phân vùng $\mu_{ij}$, ta tính đạo hàm riêng của \eqref{eq:Lagrange1} với mỗi giá trị phân vùng $\mu_{ij}$ như sau
	\begin{align}\label{eq:daohamrieng}
		\frac{\partial}{\partial \mu_{ij}}J(\bm U,\Theta,\Lambda)  = \frac{1}{\omega_i}m\mu_{ij}^{m-1}D^2(\theta_i\,,f_j) + \delta_iD^*_{ij}-\delta_im\mu_{ij}^{m-1}D^*_{ij}-\lambda_i.
	\end{align}
	Cho đạo hàm riêng \eqref{eq:daohamrieng} bằng $0$, ta có
	\begin{align*}
		m\mu_{ij}^{m-1}\left(\frac{1}{\omega_i}D^2(\theta_i\,,f_j) -\delta_iD^*_{ij}\right ) = \lambda_i - \delta_iD^*_{ij} \Leftrightarrow 	\mu_{ij} =  	\left[\frac{(\lambda_i - \delta_i D^*_{ij}) \omega_j}{m\left(D^2(\theta_i\,,f_j)- f_j \delta_i D^*_{ij}\right)}\right]^{\frac{1}{m-1}}.
	\end{align*}
	Đặt $\delta_i=\delta'\lambda_i$, khi đó $\mu_{ij}$ trở thành 
	\begin{align}\label{eq:mu}
		\mu_{ij} =  	\lambda_i^{\frac{1}{m-1}}\left[\frac{(1- \delta' D^*_{ij}) \omega_j}{m\left(D^2(\theta_i\,,f_j)- f_j \delta_i D^*_{ij}\right)}\right]^{\frac{1}{m-1}}.
	\end{align}
	Ngoài ra, đối với các nhân tử $\lambda_i$,  ta tính đạo hàm riêng của \eqref{eq:Lagrange1} và cho đạo hàm bằng $0$, ta cũng có $\sum_{i=1}^c\mu_{ij} = 1$. Thay $\mu_{ij}$ từ công thức \eqref{eq:mu} vào kết quả vừa nhận, ta có
	\begin{align}\label{eq:lambda}
		\lambda_i^{\frac{1}{m-1}}=\frac{1}{\sum_{i=1}^c\left[\frac{(1- \delta' D^*_{ij}) \omega_j}{m\left(D^2(\theta_i\,,f_j)- f_j \delta_i D^*_{ij}\right)}\right]^{\frac{1}{m-1}}}.
	\end{align}
	
	Cuối cùng, ta thay \eqref{eq:lambda} và \eqref{eq:mu} ta được điều phải chứng minh. 
}
...
\bode{\label{lem2}
	Cho $\psi\,:\mathbb{R}^{cp} \to \mathbb{R},\psi(\Theta) = J_{EM-IFCF}(\bm{\mu},\Theta)$ là bình phương khoảng cách xung quanh trọng tâm được tính khi $\bm{\mu}$ là hằng số ($\bm{\mu}\in\bm U_{f}$). Khi đó, $\Theta^*$ là nghiệm cực tiểu của $\psi$ khi và chỉ khi $\Theta^* = G(\bm{\mu})$ được cho {bởi (\ref{trongtamEMIFCF}).}
	\begin{align}\label{trongtamEMIFCF}
		\theta_i = \frac{\sum_{j=1}^{n}(\mu_{ij})^m f_j}{\sum_{j=1}^{n}(\mu_{ij})^m} \,,1\le i \le c.
	\end{align}
	}

\chm{
	Vì việc tối thiểu hóa $\psi$ trên $\mathbb{R}^{cp}$ là một bài toán không bị ràng buộc, với mỗi $f_j$, khi xét ma trận Hessian, ta dễ dàng suy ra rằng $\bm H_\psi(\Theta)$ là một ma trận đường chéo sau khi vi phân hàm mục tiêu \eqref{eq:EMIFCF} theo $\theta_i$ được xác định bởi
	\begin{align*}
		\frac{\partial^2\psi}{\partial\theta_{t}\partial \theta_{s}}=\left\{\begin{array}{ll}
			\frac{2}{\omega_i}\sum_{j=1}^n(\mu_{tj})^m,& \text{với mọi } t=s\\
			0,& \text{ngược lại}
		\end{array}\right.
	\end{align*}
	
	Vì $\bm{\mu}\in\bm U_f$, ta có $\sum_j\mu_{ij}>0$, suy ra $\sum_j\mu_{ij}^m>0$ với mọi $i$. Điều này chứng minh $\bm H_\psi(\Theta)$ là một ma trận xác định dương với mọi $\Theta$. Do đó, $\psi(\Theta)$ là một hàm lồi nghiêm ngặt trong $\mathbb{R}^{cp}$. Do đó, $\Theta^*$ là một nghiệm cực tiểu toàn cục nghiêm ngặt khi và chỉ khi $$ \frac{\partial \psi}{\partial \theta_{i}}(\Theta^*)=\frac{2}{\omega_i}\sum_{j=1}^n(\mu_{ij})^m(f_j-\theta_i^*) = 0, $$ tương đương với $\Theta^*=G(\bm{\mu})$ với công thức~\eqref{trongtamEMIFCF}.
}

Tuy nhiên, hai bổ đề trên chỉ là điều kiện cần để $(\bm{\mu}^*\,,\Theta^*)$ là một cực tiểu toàn cục của hàm mục tiêu $J_{EMIFCF}$. Từ đây, câu hỏi tiếp theo là liệu dãy $\{T_{EM-IFCF}^{(t)}(\bm{\mu}^{(0)})\}$ có hội tụ về $\bm{\mu}^*$ hay không. Điều kiện đủ cho định lý Zangwill là tính com-pắc của một tập con $\bm{U}_f \times \mathbb{R}^{cd}$
chứa tất cả các dãy sinh bởi $T_{EM-IFCF}$. 

 
 

Ba định lý này là một phần của chứng minh \eqref{eq:EMIFCF} bằng định lý Zangwill, chứng minh trực tiếp cho kết quả của Định lý~\ref{theo4}.

\dly{[Ràng buộc com-pắc]\label{theo3}
	Cho $[\mathrm{conv}(\mathcal{F})]^c$ là tích Descartes bậc $c$ của bao lồi $\mathcal{F}$, và $(\bm{\mu}^{(0)}\,,\Theta^{(0)})$ là điểm bắt đầu của phép lặp với $J_{EM-IFCF}$ với $\bm{\mu}^{(0)}\in\bm{U}_f$ và $\theta^{(0)} = G(\bm{\mu}^{(0)})$. Khi đó
	$T^{(0)}_{EM-IFCF}\left(\bm{\mu}^{(0)}\,,\Theta^{(0)}\right)\in \bm{U}_f\times [\mathrm{conv}(\mathcal{F})]^c,$
	và $\bm{U}_f\times [\mathrm{conv}(\mathcal{F})]^c$ là com-pắc trong $\bm{U}_f \times \mathbb{R}^{cn}$.
}

\chm{
	Giả sử $\bm{\mu}^{(0)}\in\bm{U}_f$, khi đó $\theta^{(0)} = G(\bm{\mu}^{(0)})$ được tính theo \eqref{eq:emifcf_u}. Khi đó $$\theta^{(0)}_i = \frac{\sum_{j=1}^n\left(\mu^{(0)}_{ij}\right)^mf_j}{\sum_{j=1}^n\left(\mu^{(0)}_{ij}\right)^m}.$$
	Cho $\rho_{ij} = \frac{\left(\mu^{(0)}_{ij}\right)^m}{\sum_{l=1}^n\left(\mu^{(0)}_{il}\right)^m}\,, 1\le j\le n$. Theo các ràng buộc \eqref{c1_pro}, thì $0<\rho_{ij}<1$, $\forall i\,, j$. Vậy $\forall i$, ta viết lại $\theta^{(0)}_i = \sum_{j=1}^n\rho_{ij}f_j$, với $\sum_{j=1}^n\rho_{ij}=\sum_{j=1}^n\left(\frac{\left(\mu^{(0)}_{ij}\right)^m}{\sum_{l=1}^n\left(\mu^{(0)}_{il}\right)^m}\right)=\frac{\sum_{l=1}^n\left(\mu^{(0)}_{il}\right)^m}{\sum_{l=1}^n\left(\mu^{(0)}_{il}\right)^m}=1.$
	
	Tóm lại $\forall i\,, \theta^{(0)}_i\in \mathrm{conv}(\mathcal{F})$, và do đó $\theta^{(0)}\in [\mathrm{conv}(\mathcal{F})]^c$. Tiếp tục đệ quy  $\bm{\mu}^{(1)}=F(\bm{\mu}^{(0)})\in \bm{U}_f$ theo \eqref{eq:emifcf_u}, và $\theta^{(1)}=G(\bm{\mu}^{(1)})\in [\mathrm{conv}(\mathcal{F})]^c$ tương tự lập luận như trên. Do đó, mọi dãy lặp của $J_{EM-IFCF}$ đều thuộc về $\bm{U}_f\times [\mathrm{conv}(\mathcal{F})]^c$ với mọi $t\ge1$. Mặc dù ta có thể khởi tạo $\theta^{(0)}\in\mathbb{R}^{cd}\setminus\mathrm{conv}[(\mathcal{F})]^c$, $\bm{\mu}^{(0)} = F(\theta^{(0)})\in\bm{U}_f$. Do đó, $\theta^{(t)} = G(\bm{\mu}^{(t)}) \in [\mathrm{conv}(\mathcal{F})]^c\,,\forall t\ge1$. Rõ ràng, $\bm{U}_f\times [\mathrm{conv}(\mathcal{F})]^c$ là một tập com-pắc trong $\mathcal{F}$ hội tụ hữu hạn.
}

\dly{\label{theo1}
	Xét $S_{EM-IFCF} = \{(\bm{\mu}^*,\Theta^*)\,:\, J_{EM-IFCF}(\bm{\mu}^*, \Theta^*) < J_{EM-IFCF}(\bm{\mu}, \Theta)\,, \forall (\bm{\mu}, \Theta)\in B^0((\bm{\mu}^*, \Theta^*), r)\}$ là tập nghiệm. Khi đó, $J_{EM-IFCF}$ là hàm giảm của $\{T_{EM-IFCF}\,,S_{EM-IFCF}\}$.
}

\chm{
	Đầu tiên, vì $\{y \to \Vert y\Vert ^2\}$, $\{y \to 1 - y\}$, và $\{y \to y^m\}$ là liên tục, và $J_{EM-IFCF}$ là tổng các tích các hàm. Do đó, $J_{EM-IFCF}$ liên tục trên $\bm{U}\times\mathbb{R}^{cd}$. Tiếp theo, giả sử $(\bm{\mu}\,, \Theta) \notin S_{EM-IFCF}$. Từ \eqref{TO} suy ra \begin{align*}
		\begin{array}{cll}
			J_{EM-IFCF}(T_{EM-IFCF}(\bm{\mu}, \Theta)) &= J_{EM-IFCF}\left(T_2\circ T_1(\bm{\mu}\,,\Theta)\right) &\\
			&= J_{EM-IFCF}(F\circ G(\bm{\mu}), G(\bm{\mu})) & \text{theo Định nghĩa } \ref{TO}\\
			&< J_{EM-IFCF}(\bm{\mu}\,, G(\bm{\mu})) & \text{theo Bổ đề } \ref{lem1}\\
			&< J_{EM-IFCF}(\bm{\mu}\,, \Theta) & \text{theo Bổ đề } \ref{lem2}
		\end{array}
	\end{align*}
	
	Cuối cùng, Bổ đề~\ref{lem1} và Bổ đề~\ref{lem2} suy ra rằng $(\bm{\mu}, \Theta) \in S_{EM-IFCF}$.
}

\dly{[Ràng buộc liên tục]\label{theo2}
	$T_{EM-IFCF}$ liên tục trên $\bm{U}_f \times \mathbb{R}^{cn}$.
}
\chm{
	Vì $T_{EM-IFCF} = T_2\circ T_1$ và hợp các hàm liên tục nên cũng liên tục. Khi này, ta chỉ cần $T_1$ và $T_2$ đều liên tục. Vì $T_1 (\bm{\mu}, \Theta) = G(\bm{\mu})$, $T_1$ liên tục nếu $G$ liên tục. Để chứng minh $G$ liên tục trong $cn$ biến $\bm{\mu}$, ta có $G$ là một trường véc-tơ, với độ phân giải của trường vô hướng $(cd)$ là $G\,:\,\mathbb{R}^{cd}\to \mathbb{R}^{cp}$. Bây giờ $\{\bm{\mu}\to \bm{\mu}^m\}$ là liên tục, $\{\bm{\mu}^m\to \bm{\mu}^mx_{ik}\}$ là liên tục. Do đó, $G_{ik}$ là thương của hai hàm liên tục nên cũng liên tục với mọi $(i\,, k)$. Do đó, $G$ và $T_1$ liên tục trên toàn bộ tập xác định.
	
	Tương tự, vì $T_2(\Theta) = (F(\Theta)\,, \Theta)$, nên chỉ cần chứng minh $F$ là một hàm liên tục trên biến $\{\theta_i\}$. Cũng có $F$ là một trường véc-tơ có độ phân giải bởi $(cd)$ với vô hướng $F\,:\,\mathbb{R}^{cp}\to \mathbb{R}^{cd}$. Vì $\{\theta_i \to D^2(\theta_i\,,x_j)\}$ liên tục với mọi $i$, và $\{D^2(\theta_i\,,x_j)\to D(\theta_i\,,x_j)^{-2/(m-1)}\}$ liên tục. Do đó, $F_{ij}$ là thương của hai hàm liên tục với giả thuyết $D^2>0$. Do đó, $F$ và $T_2$ liên tục trên toàn bộ tập xác định. Cuối cùng, $J_{EM-IFCF} = T_2\circ T_1$ liên tục trên $\bm{U}_f\times \mathbb{R}^{cd}$.
}
	
Tập hợp các kết quả của các bổ đề và định lý trên thành một định lý về sự hội tụ của EM-IFCF.

\dly{[Định lý hội tụ cho $J_{EM-IFCF}$]\label{theo4}
	Xét tập hợp $\mathcal{F} = \{f_1\,,f_2\,,\ldots\,,f_n\}\subset \mathbb{R}^d$, cho hàm mục tiêu $J_{EM-IFCF}(\bm{U}\,,\Theta)$ có dạng \eqref{eq:EMIFCF}, trong đó $\bm{U}$ thỏa mãn \eqref{c1_pro} và $\Theta=\{\theta_1\,,\theta_2\,,\ldots\,,\theta_c\}$. Nếu $T_{EM-IFCF}\,:\,\mathbb{R}^{cd} \times \bm{U}_f\to \mathbb{R}^{cd} \times \bm{U}_f$ là một toán tử lặp thuật toán (Picard) của $J_{EM-IFCF}$, và với mọi $t$ sao cho $D^2(\theta_i^{(t)}\,,f_j)>0$ thì với mọi $(\bm{\mu}^{(0)}\,,\Theta^{(0)})\in \bm{U}_f\times [\mathrm{conv}(\mathcal{F})]^c$, hoặc
	\begin{enumerate}
		\item Dãy $\{(J_{EM-IFCF})^{(t)}(\bm{\mu}^{(0)}, \Theta^{(0)})\}$ dừng tại giá trị cực tiểu cục bộ $(\mu_{ij}^*, \Theta^*)$ của $J_{EM-IFCF}$; hoặc
		\item Dãy n$\{(J_{EM-IFCF})^{(t)}(\bm{\mu}^{(0)}, \Theta^{(0)})\}$ chứa một dãy con sao cho $\{(J_{EM-IFCF})^{(t_k)}(\bm{\mu}^{(0)}\,, \Theta^{(0)})\}\to (\bm{\mu}^*, \Theta^*)$ là giá trị cực tiểu cục bộ của $J_{EM-IFCF}$ khi $t_k\to\infty$.
		
	\end{enumerate}
}

\chm{
	Do hàm mục tiêu $J_{EM-IFCF}$ liên tục trên $\bm{U}_f\times \mathbb{R}^{cd}$, Định lý~\ref{theo1} chỉ ra rằng $J_{EM-IFCF}$ là một hàm Zangwill giảm cho tập nghiệm $\{T_{EM-IFCF}\,,S_{EM-IFCF}\}$ trong đó $S_{EM-IFCF}$ là tập các cực tiểu nghiêm ngặt của $J_{EM-IFCF}$. Định lý~\ref{theo2} khẳng định thuật toán giảm lặp $J_{EM-IFCF}$ liên tục trên $\bm{U}_f\times [\mathrm{conv}(\mathcal{F})]^c$ và theo Định lý~\ref{theo3}, toán tử dãy lặp $J_{EM-IFCF}$ luôn nằm trong một tập con com-pắc của miền xác định. Kết quả suy ra ngay từ định lý Zangwill.
}
...
\begin{table}[H]
	...
	\begin{tabular*}{\textwidth}{@{\extracolsep\fill}lp{4cm}p{11cm}@{\extracolsep\fill}}
		...
		2009 & Ren Jiangtao \textit{et al.} \cite{ren2009naive} & Naive Bayes cho dữ liệu bất định: ước lượng mật độ điều kiện lớp bằng kỳ vọng kernel trên PDF; ba biến thể AVG/SBC/FBC (FBC thường tốt nhất). \\
		2009 & Tsang Smith \textit{et al.} \cite{tsang2009decision} & Cây quyết định, chia nút bằng entropy kỳ vọng từ các “tuple phân số” khi tách ngưỡng. \\
		2010 & Ge Jiaqi, Xia Yuni \& Nadungodage Chandima \cite{ge2010unn} & Mạng thần kinh xử lý trực tiếp phân bố đầu vào, mở rộng MLP và tối ưu để tăng tốc. \\
...
Trích xuất đặc trưng là đầu vào cho các mô hình xử lý hình ảnh trong nhận dạng thống kê. 
