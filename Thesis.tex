...
Đáng chú ý, trường hợp (a) và (b) của hai loại khoảng cách $\mathcal{L}_2$ và $D_W$ là trái ngược nhau. Khi thay đổi hình dáng, khoảng cách $\mathcal{L}_2$ giảm nhưng $D_W$ lại tăng lên. 
}
...
\textbf{Bài giải.} Với các thông số ban đầu $c=3$, $\epsilon=10^{-3}$ và khoảng cách $\mathcal{L}_1$, ta tiến hành thuật toán phân tích chùm mờ. Giả sử hàm mật độ xác suất trọng tâm $\theta_i(x)$ ở ba chùm lần lượt là $\theta_1{(x)} \sim \mathcal{N}(0,1)\,, \theta_2{(x)} \sim \mathcal{N}(1,1)$ và $\theta_3{(x)} \sim \mathcal{N}(2,1)$
