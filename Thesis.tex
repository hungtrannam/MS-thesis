...
Đây là khái niệm quan trọng trong luận văn này. Để cụ thể, chúng tôi tập trung khảo sát tính chất của hàm mật độ ($\mu$ là độ đo Lebesgue) liên tục tuyệt đối của biến ngẫu nhiên $X$. Các hàm mật độ cũng được định nghĩa trên tập số thực. 
...
\begin{align*}
		\mathbb{E}(X) &= \int_{\Omega} x f(x) \mathrm{\,d}x <\infty\,,\\
...
\begin{figure}[H]
	\centering
	    ...
	    \begin{flushleft}
	    	\it\footnotesize Chú thích: Hàm mật độ đại diện cho ba hàm Gauss minh họa bằng nét đứt, thể hiện bằng đường thẳng in đậm (a) dựa vào trọng số phân bổ đều giữa ba hàm $\mu_{ij}=1/3$, và (b) dựa vào vị trí tối ưu. Nguồn: Minh họa của tác giả
	    \end{flushleft}
...
Khoảng cách $p$-Wasserstein  đo mức chênh lệch tối ưu giữa hai phân phối xác suất, xét trên tất cả các phép ghép cặp khả dĩ. Ở đây, chúng tôi sử dụng $p=2$ để tính toán đo độ khác biệt giữa hai mật độ xác suất liên tục. Trong thực tế, ta tính toán cụ thể khoảng cách này bằng cách biểu diễn nó qua hàm phân vị. Ta có Định lý~\ref{dly:phanvi}.

\dly{\label{dly:phanvi}
	Khoảng cách $p$-Wasserstein giữa hai độ đo xác suất $\mu$ và $\nu$ có thể được biểu diễn dưới dạng phân vị trong trường hợp $1$-chiều. Tức là,
	$$
...
\dly{[Khoảng cách $2$-Wasserstein cho hàm mật độ Gauss 1 chiều]
...
\dly{[Khoảng cách $2$-Wasserstein cho hàm mật độ Gauss $d$-chiều]
...
\vdu{[So sánh các loại khoảng cách hàm mật độ] Xét hai hàm mật độ xác suất đều $f_1(x)$ và $f_2(x)$. Hình~\ref{fig:daydeu} mô tả bốn tình huống khoảng cách giữa hai hàm mật độ này.
	\begin{figure}[H]
		...
		\footnotesize \it Chú thích: Bốn trường hợp mật độ đều với (a) hai hàm rời nhau, cùng chiều rộng; (b) hai hàm rời nhau, khác chiều rộng; (c) hai hàm rời nhau, cách xa nhau; và (d) hai hàm chồng lấp. Ở đây, ô vuông trắng \begin{tikzpicture}
			\node[draw=black,line width=1pt,opacity=0.7, minimum size=3mm] () {};\end{tikzpicture} biểu thị hàm mật độ $f_1(x)$ và ô vuông xám  \begin{tikzpicture}
			\node[draw=black,line width=1pt,opacity=0.7, fill=black!45, minimum size=3mm] () {};\end{tikzpicture}  biểu thị hàm mật độ $f_2(x)$. Nguồn: Minh họa của tác giả.
		\end{flushleft}
	...
	\begin{itemize}
		\item $\mathcal{L}_1(f_1\,,f_2) = \int\vert f_1 - f_2\vert \mathrm{\,d}x = \int \left(1\times \bm 1_{[2\,,3]}(x) + 1\times \bm 1_{[5\,,6]}(x)\right)\mathrm{\,d}x\ = \int_2^31\mathrm{\,d}x + \int_5^61\mathrm{\,d}x=2$,
		\item  $\mathcal{L}_2(f_1\,,f_2) = \left(\int(f_1 - f_2)^2 \mathrm{\,d}x\right)^{1/2} = \left(\int_2^31^2\mathrm{\,d}x + \int_5^61\mathrm{\,d}x\right)^{1/2}=\sqrt{2}$,
		\item $D_B(f_1\,,f_2)$ không xác định hữu hạn, tiến ra dương vô cùng vì $f_1(x)f_2(x) = 0\,,\forall x$,
		\item $D_H(f_1\,,f_2)  = \left(1-f_1(x)f_2(x)\right)^{1/2} = \sqrt{1-0} = 1$,
		\item $D_W(f_1\,,f_2) = \vert \mathbb{E}_{f_1} - \mathbb{E}_{f_2}\vert = |5,5-2,5|=3$.
			\end{itemize}
...
\begin{table}[H]
	...
	\end{tabular*}
	\begin{flushleft}
		\it{\footnotesize Chú thích: \footnotemark[1] khoảng cách Bhattacharyya; \footnotemark[2] khoảng cách Hellinger; \footnotemark[3] khoảng cách Wasserstein. Nguồn: Tác giả tổng hợp}
	\end{flushleft}
\end{table}
...
\begin{figure}[H]\footnotesize
	...
	\begin{flushleft}
	\footnotesize	{\textit{Nguồn: Tác giả vẽ lại từ Gao et al. \cite{gao2023overview}}}
	\end{flushleft}
...
\textbf{Bài giải.} Với các thông số ban đầu $c=3$, $\epsilon=10^{-3}$ và khoảng cách $\mathcal{L}_1$, ta tiến hành thuật toán phân tích chùm mờ. Giả sử hàm mật độ xác suất trọng tâm $\theta_i(x)$ ở ba chùm lần lượt là $\theta_1{(x)} \sim \mathcal{N}(0,1)\,, \theta_2{(x)} \sim \mathcal{N}(3,1)$ và $\theta_3{(x)} \sim \mathcal{N}(7,1)$

Tại vòng lặp đầu tiên $t=1$, ta tính khoảng cách giữa 
