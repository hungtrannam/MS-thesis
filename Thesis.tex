...
\chm{
	...
	(\mu_{ij})^{m}(\mu_{ij})^{m}\nabla\theta_iD^2(\theta_i\,,f_j)
	\end{align}
	Cho $\nabla_{\theta_i}\psi_i(\theta_i)=\bm 0$, ta được công thức cập nhật  \eqref{eq:theta_update} đối với riêng trường hợp $D^2=\mathcal{L}_2^2(\theta_i\,,f_j)$.
...
Bảng~\ref{tab:KCTT} trình bày các công thức cập nhật trọng tâm $\theta$ của chùmtheo những khoảng cách khác nhau. 
