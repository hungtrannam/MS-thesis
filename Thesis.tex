...
\chy{	 Hàm mật độ đại diện Gauss là hàm trung bình Fréchet nên có nghiệm duy nhất \cite{frechet1948elements}. Khác với \eqref{daidien}, hàm mật độ này chỉ cần tìm một hàm Gauss duy nhất mà không cần tổ hợp các $f_j$ giúp tăng khả năng khái quát hóa. Ta cũng có phương sai Fréchet $\mathrm{v}_j(x)=\mathbb{E}(D^2(\mathcal{N}(\mu\,,\sigma)\,,f_j(x)))$.
	}
...
\begin{figure}[H]
	...
	\begin{flushleft}
		\it\footnotesize
		Chú thích: Vùng xám là phân phối thực tế $X\sim 0,3\mathcal{N}(0\,;1) + 0,7\mathcal{N}(5\,;1)$, cùng một tham số trơn, các hàm hạt nhân có ảnh hưởng đến độ mượt và khả năng khớp với hàm mật độ thực tế. Nguồn: Minh họa của tác giả.
	\end{flushleft}
...
Ở Hình~\ref{fig:dayKC}, chúng tôi ghi nhận giá trị khoảng cách $f_1\sim\mathcal{N}(0\,,1)$ và $f_2\sim \mathcal{N}(\mu\,,1)$ với $\mu$ trượt từ $-10$ đến $20$. Ta thấy được tính bị chặn của các loại khoảng cách này đúng với lý thuyết tại Bảng~\ref{tab1:KC_PDF}.
...
Phương pháp Bayes đã được nhiều nhà thống kê quan tâm cho dữ liệu hàm mật độ xác suất tại \cite{}. Một dữ liệu hàm mật độ có hai lớp $Y=0$ hoặc $1$ và một hàm mới $h(x)$ cần phân loại. Khi đó, xác suất hậu nghiệm Bayes để xếp hàm mật độ $h$ vào một trong hai lớp này là
$$
...
%\[
%f(h \,|\, \mathcal{F}) = \mathbb{E}_{X\sim h}(f_j(X))=\frac{1}{\#\mathcal{F}} \sum_{i=1}^{\#\mathcal{F}} \int_{\Omega} f_j(x) h(x) \mathrm{\,d}x.
%\]
%
...
Hàm mật độ xác suất được xem là dạng dữ liệu đặc trưng\index{dữ liệu đặc trưng (symbolic data)} với cấu trúc phức tạp hơn. Phần lớn các công trình hiện nay tập trung vào mô phỏng chùm hàm mật độ \cite{nguyen2023globally,vo2024supervised}. Trong thực tiễn, dữ liệu $1$-chiều cho phép khám phá quy luật phân bố, chẳng hạn nghiên cứu về Cá hồi vây vàng (\textit{Yellowfin tuna}) của \cite{minami2024regression} đã thể hiện phân bố chiều dài cá theo từng ô tọa độ tại Thái Bình Dương. Bên cạnh đó, các phân phối liên quan đến phân phối tỷ giá theo ngày, khí hậu ở các trạm theo năm, hay mật độ tuổi giữa hai giới, chỉ số sức khỏe theo độ tuổi \cite{irpino2015basic,billard2012symbolic} cũng là ví dụ tiêu biểu cho dạng $1$-chiều. Đặc biệt, dữ liệu hàm mật độ còn có thể được xây dựng từ hình ảnh thông qua việc trích xuất màu sắc \cite{nguyen2024new,vo2024supervised}. Trong khi đó, dữ liệu $2$-chiều thường xuất hiện trong các nghiên cứu về mối tương quan, ví dụ phân tích điểm số sinh viên giữa các khoa/viện tại Đại học Cần Thơ \cite{vovan2019cluster}. Tuy vậy, và quá trình thu thập thường mất nhiều thời gian. Hơn nữa, không gian vô hạn chiều khiến ước lượng khó lưu trữ. Do đó, nhiều phương pháp tiếp cận mới từ nhận dạng thống kê đã giải quyết nhu cầu phân tích dạng dữ liệu này đối với thực tế. Chúng bao gồm phân tích chùm và phân loại hàm mật độ xác suất. 
...
Phân tích chùm là một nhánh chính của nhận dạng không được giám sát \cite{jain2010data,saxena2017review,dinh2025categorical}. Nó có nhiệm vụ khám phá dựa vào sự tương tự giữa các mẫu trong dữ liệu đầu vào \cite{Bishop:DeepLearning24,webb2003statistical}. Ví dụ như khi ta cần khoanh vùng khối u trong hình ảnh MRI,  hay trong các báo cáo định tính cần nhóm/tổng hợp ý kiến hay quan điểm theo các chủ đề khác nhau. Nhiều họ các phân chùm có cách phân chia khác nhau, nhưng mục tiêu chung là sự phân bổ tối tiểu khoảng cách phần tử trong chùm trong khi vẫn giữ tối đa khoảng cách phần tử giữa các chùm. 
% Ta cũng có phân tích chùm các hàm mật độ xác suất là bài toán phân tích chùm cho đối tượng các hàm mật độ xác suất  
Các thuật toán này được chia thành hai loại dựa theo lý thuyết mờ \cite{bezdek2013pattern} là phân chùm không mờ (bao gồm $k$-mean \cite{nguyen2024new}, phân cấp thứ bậc\index{phân cấp thứ bậc (hierarchical clustering)} và phân chùm tự cập nhật\index{phân chùm tự cập nhật (self-updating process)}) và phân chùm mờ (bao gồm Fuzzy $c$-means \cite{nguyentrang2017fuzzy,vovan2019cluster}). 



Trong nước, Bảng~\ref{tab:pdf_clustering_summary} cho thấy ba xu thế nổi bật: (i) dịch chuyển từ mô hình cứng sang mờ/khả dĩ và bán giám sát; (ii) từ độ đo hình học đơn giản sang độ đo thích nghi không gian hàm mật độ; và (iii) từ $c$-chùm cố định sang tự động hoá nhờ tối ưu hoá metaheuristic. Giai đoạn khởi đầu (2019) tập trung vào phân chùm cứng với độ đo chuyên biệt CWD \cite{vovan2019cluster}, song đã sớm mở rộng bằng các chiến lược tiến hoá (DE) để vừa phân chùm vừa xác định số chùm. Từ 2020–2021, các phương pháp mờ và entropy xuất hiện, khai thác đa dạng độ đo ($\mathcal{L}_1$, $\mathcal{L}_2$, kernel Gauss) nhằm nắm bắt khác biệt hình dạng của phân bố. Trong giai đoạn 2022–2023, các tiếp cận mờ được củng cố bởi mục tiêu cân bằng và đa mục tiêu \cite{nguyen2023balance}, đồng thời được tích hợp metaheuristic cải tiến \cite{nguyen2023globally}, hướng đến tự động hóa toàn diện. Đến năm 2024, hai nhánh mới được ghi nhận bao gồm phân tích chùm khả dĩ nhằm tăng cường khả năng nhận diện ngoại lệ và hướng mờ bán giám sát khai thác nhãn hạn chế để nâng cao ổn định khi ranh giới chùm chồng lấp.
...
\begin{table}[H]
	...
	\begin{tabular*}{\columnwidth}{@{\extracolsep\fill}lllll@{\extracolsep\fill}} 
		\toprule
		\textbf{Năm} & \textbf{Loại mô hình} & \textbf{Phương pháp} &\textbf{Khoảng cách} & \textbf{Tự động số chùm} \\
		\midrule
	\cite{vovan2019cluster}	2019 & Cứng & k-Medoids &  CWD-based & Không \\
		
		2019 & Phân chùm cứng & Binary elitist DE &  $\mathcal{L}_2$ & Có \\
		
		2020 & Phân chùm mờ & Genetic Alg. &  $\mathcal{L}_1$ & Có \\
		
		2020 & Entropy & Jackknife entropy &$\mathcal{L}_1$ Kernel Gauss & Không \\
		
		2021 & Phân chùm mờ& Kernel K-Medoids & $\mathcal{L}_1$, Kernel Gauss & Không \\
		
	\cite{nguyen2023balance}	2022 & Phân chùm mờ & Balance-driven + MO &  $\mathcal{L}_2$  & Có \\
		
		2023 & Phân chùm mờ & Prototype-based DE &$\mathcal{L}_1$  Kernel Gauss & Có \\
		
	\cite{nguyen2023globally}	2023 &Phân chùm mờ & GA cải tiến &  $\mathcal{L}_1$ & Có \\
		
		2024 & phân chùm khả dĩ & Possibilistic objective & $\mathcal{L}_2$ & Không \\
		
		2024 & Phân chùm mờ bán giám sát & Fuzzy + bán giám sát & $\mathcal{L}_1$ & Có \\
		\bottomrule
	\end{tabular*}
...
Phương pháp phân loại cho hàm mật độ xác suất là một bài toán mới mẻ nên còn nhiều thách thức lớn cả bề sâu và bề rộng \cite{}. 

Nhìn chung cho đến nay, hai phương pháp chính để phân loại là từ hai góc nhìn. Xem hàm mật độ là một véc-tơ hữu hạn chiều hay hàm vô hạn chiều. 
...
\begin{description}
	\item[Rand Index] Rand Index đo mức độ tương đồng giữa \(\mathcal{P}\) và \(\mathcal{G}\). Ta có \(RI=1\) nếu phân chùm trùng hoàn toàn với nhãn thực tế, được định nghĩa là
	$
	RI = \frac{a+d}{a+b+c+d},
	$
