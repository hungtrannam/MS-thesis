...
\begin{figure}[H]
	\centering
	    ...
	    \end{flushleft}
	\caption{Minh họa hai hàm mật độ xác suất đại diện một chùm hàm mật độ}
...
\dly{[Khoảng cách $2$-Wasserstein giữa hai hàm mật độ Gauss 1 chiều]
	Giả sử $f_1(x) \sim \mathcal{N}(\mu_1, \sigma_1^2)$ và $f_2(x) \sim \mathcal{N}(\mu_2, \sigma_2^2)$ là hai phân bố Gauss một chiều. Khi đó, khoảng cách $2$-Wasserstein giữa chúng là
	$
...
\dly{[Khoảng cách $2$-Wasserstein giữa hai hàm mật độ Gauss $d$-chiều]
	Giả sử $f_1(x) \sim \mathcal{N}(\mu_1, \Sigma_1)$ và $f_2(x) \sim \mathcal{N}(\mu_2, \Sigma_2)$ là hai phân bố Gauss trong $\mathbb{R}^d$. Khi đó, khoảng cách $2$-Wasserstein giữa chúng là
	$
...
\dly{[Khoảng cách $2$-Wasserstein giữa hai phân phối đều $1$-chiều] Giả sử $f_1(x) \sim \mathcal{U}[a_1, b_1]$ và $f_2(x) \sim \mathcal{U}[a_2, b_2]$ là hai phân phối đều một chiều. Khi đó, khoảng cách $2$-Wasserstein giữa chúng là
		$
...
\vdu{[So sánh các loại khoảng cách hàm mật độ] Xét hai hàm mật độ xác suất đều $f_1(x)$ và $f_2(x)$. Hình~\ref{fig:daydeu} mô tả bốn tình huống khoảng cách giữa hai hàm mật độ này.
	\begin{figure}[H]
		...
		\footnotesize \it Chú thích: Bốn trường hợp mật độ đều với (a) hai hàm rời nhau, cùng chiều rộng; (b) hai hàm rời nhau, khác chiều rộng; (c) hai hàm rời nhau, cách xa nhau; và (d) hai hàm chồng lấp. Ở đây, ô vuông trắng \begin{tikzpicture}
			\node[draw=black,line width=1pt,opacity=0.7,fill=black!15, minimum size=3mm] () {};\end{tikzpicture} biểu thị hàm mật độ $f_1(x)$ và ô vuông xám  \begin{tikzpicture}
			\node[draw=black,line width=1pt,opacity=0.7, fill=black!55, minimum size=3mm] () {};\end{tikzpicture}  biểu thị hàm mật độ $f_2(x)$. Nguồn: Minh họa của tác giả.
		\end{flushleft}
...
\vdu{
Hai hạt nhân cơ bản cho khoảng cách hai hàm mật độ $f_1(x)$ và $f_2(x)$ là hạt nhân chuẩn
	$
	K_G(f_1, f_2) = \exp\left(-\frac{D^2(f_1\,,f_2)}{2\sigma^2}\right),
	$
	và {hạt nhân Hellinger} 
	$
	K_H(f_1, f_2) = \int_\Omega \sqrt{f_1(x) f_2(x)}\mathrm{\,d}x.
	$
}

Ở đây, việc chuyển đổi từ hàm hạt nhân đến khoảng cách được cho bởi $D = 1-K$ hoặc $D=2(1-K)$.
...
Khi hai ta cần tính khoảng cách từ chùm hàm $\mathcal{F}\cup\mathcal{G}$ đến một chùm hàm $\mathcal{H}$, ta cũng có thể sử dụng các loại liên kết trên. Trong thực tế, các liên kết $\Delta$ được biến đổi thành
\begin{align*}
...
Trong khoa học máy tính, các khoảng cách giữa các hàm mật độ xác suất thường chứa các tích phân xác định, vốn không thể tính chính xác bằng phương pháp thông thường. Do đó, các tích phân này cần xấp xỉ bằng phương pháp số. Luận văn này, chúng tôi chọn sử dụng phương pháp hình thang\index{phương pháp hình thang (trapezoidal method)} nhằm cải thiện độ chính xác so với tổng Riemann. Mặc dù Simpson hoặc Monte-Carlo có thể đạt độ chính xác cao hơn, nhưng chúng đòi hỏi chi phí tính toán và triển khai phức tạp hơn. Một tích phân trên miền $\Omega \subset \mathbb{R}^d$ được xấp xỉ bằng công thức tổng quát
...
\end{align}
trong đó, $\mu_{ij}$ là biến tiềm ẩn thể hiện mức độ thành viên của hàm mật độ $f_j(x)$ thỏa $\mu_{ij}= \bm 1_{j\in c_i}$. Tức là $\mu_{ij} =1$ nếu hàm $j$ thuộc chùm $c_k$ và $\mu_{ij}=0$ nếu ngược lại.  Hàm $\theta_i(x)$ là hàm mật độ trọng tâm của chùm thứ $c_i$. Ký hiệu $\mathrm{D}^2(\theta_i\,,{f}_j)$ là khoảng cách giữa hàm mật độ trọng tâm đến các đối tượng $f_j(x)$ còn lại trong dữ liệu. 
...
\textbf{Bài giải.} Đầu tiên, ta tính toán các khoảng cách từng đôi hàm mật độ bất kỳ bằng khoảng cách $\mathcal{L}_2(f_i\,,f_j)$ với $i\,,j = 1\,,\ldots\,,7$, ta nhận được ma trận khoảng cách
$$\bm D=\begin{bmatrix}
	 \\
	0,74 &  \\
	0,75 & 0,75 &  \\
	\textbf{0,26} & 0,71 & 0,75 &  \\
	0,75 & 0,49 & 0,74 & 0,75 &  \\
	0,75 & 0,74 & 0,38 & 0,75 & 0,67 &  \\
	0,75 & 0,29 & 0,75 & 0,74 & \textbf{0,26} & 0,72 & 
\end{bmatrix}.$$
	...
	\begin{figure}[H]
	     ...
	     \begin{subfigure}[b]{.45\textwidth}
	         \centering
	         \includegraphics[width=.9\linewidth]{Figs/tree_L2_single.pdf}
	         \caption{Liên kết đủ}
	     \end{subfigure}
	     \begin{subfigure}[b]{.45\textwidth}
	     	\centering
...
Mục tiêu tiếp theo là ước lượng $\beta_0$ và hàm $\beta(x)$. Do $\beta(x)$ là vô hạn chiều, ta xấp xỉ một tập hàm cơ sở\index{hàm cơ sở (basis functions)} chọn trước để ước lượng, cụ thể là hàm Gauss, wavelet, chuỗi Fourier, đa thức Bernstein và Legendre, hoặc B-spline. Khi đó,
$$
l_i = \beta_0 + \sum_{m=1}^M \theta_m \underbrace{\int_\Omega \psi_m(x) {\mathcal{F}}_i(x)\, dx}_{=: \phi_{i,m}},
$$
trong đó $\phi_{i,m}$ là mô-men của hàm mật độ ${\mathcal{F}}_i(x)$ trên hàm cơ sở $\psi_m(x)$.
Bài toán ước lượng $\beta(x)$ chuyển thành
$$
\max_{\beta_0, \theta} \sum_{i=1}^n \left[y_i \log \pi_i + (1 - y_i)\log (1 - \pi_i)\right]
$$
