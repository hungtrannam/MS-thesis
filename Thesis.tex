...
\vdu{
Ba hạt nhân cơ bản cho khoảng cách hai hàm mật độ $f_1(x)$ và $f_2(x)$ là hạt nhân chuẩn\index{hàm hạt nhân chuẩn (Gaussian kernel)}
	$
	K_G(f_1, f_2) = \exp\left(-\frac{\mathcal{L}_p^2(f_1\,,f_2)}{\beta\sigma^2}\right),
	$
	trong đó $\beta\in\mathbb{R}_{>0}$, hạt nhân tuyến tính $K_L(f_1, f_2) = \int_\Omega f_1(x)f_2(x)\mathrm{\,d}x$,
	và hạt nhân Bhattacharyya\index{hàm hạt nhân Bhattacharyya (Bhattacharyya kernel)}
	$
...
Giả sử tập huấn luyện $\mathcal{F}=\{f_{i},\, i=1,\dots,n\}$ được gán nhãn theo hai lớp $Y\in\{0,1\}$. 
...
Hàm mật độ xác suất được xem là dạng dữ liệu đặc trưng\index{dữ liệu đặc trưng (symbolic data)} với cấu trúc phức tạp hơn. Nó cho phép khám phá quy luật phân bố tại nhiều tổng thể khác nhau. Do đó, nó được xem như là dữ liệu không chắc chắn do phép đo không chính xác, có độ trễ, và lỗi quyết định.
...
Bảng~\ref{tab:pdf_clustering_summary} mô tả quá trình phát triển phương pháp phân tích chùm hàm mật độ giai đoạn 5 năm từ 2019 đến 2024. Nhìn chung, các nghiên cứu cho thấy ba xu thế nổi bật cả về bề sâu và bề rộng. 
...
\begin{table}[H]
	...
	\begin{tabular*}{\textwidth}{@{\extracolsep\fill}lp{4cm}p{6cm}lp{1.5cm}@{\extracolsep\fill}} 
		\toprule
		\textbf{Năm} & \textbf{Tác giả} & \textbf{Loại mô hình} & \textbf{Khoảng cách} & \textbf{Phân chùm tự động} \\
		\midrule
