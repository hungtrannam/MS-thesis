...
% ====== CÀI ĐẶT LỀ TRANG ======
\usepackage[top=2cm,bottom=2cm,right=2cm,left=3cm,headheight=30pt]{geometry} % Cài đặt lề văn bản và chiều cao phần đầu trang
...
% Caption canh phải
\captionsetup[table]{position=top, justification=raggedright, singlelinecheck=false, font=footnotesize, labelfont=bf}
\captionsetup[figure]{position=bottom, justification=centering, singlelinecheck=false, font=footnotesize, labelfont=bf}
...
\tch{\label{tc1} Hàm mật độ thỏa mãn các tính chất sau
\begin{enumerate}
...
\dng{[Hàm mật độ Gauss $d$-chiều]
	...
	\dly{[Kỳ vọng và hiệp phương sai] Véc-tơ kỳ vọng và ma trận hiệp phương sai của biến ngẫu nhiên  $\bm X$ có hàm mật độ xác suất Gauss lần lượt là $\mathbb{E}(\bm{X})= \bm\mu$, và $\mathrm{Cov}(\bm{X}) = \bm{\Sigma}$.
	}
...
\dng{[Hàm mật độ đều]
	...
	\dly{[Kỳ vọng và phương sai] Kỳ vọng và phương sai của biến ngẫu nhiên $X$ có hàm mật độ xác suất đều lầ  lượt là $\mathbb{E}(X) = \frac{l + u}{2}$, và $\mathrm{Var}(X) = \frac{(u- l)^2}{12}$.
}
...
\dng{[Hàm mật độ đều $d$-chiều]
	...
	\dly{[Kỳ vọng và hiệp phương sai] Véc-tơ kỳ vọng và ma trận hiệp phương sai của biến ngẫu nhiên $\bm X$ có hàm mật độ xác suất đều lần lượt là $\mathbb{E}(\bm X) = \frac{\bm{l} + \bm{u}}{2}$, và $\mathrm{Cov}(\bm X) = \mathrm{diag}\left(\frac{(u_1 - l_1)^2}{12}, \dots, \frac{(u_d - l_d)^2}{12}\right)$.
}
...
\begin{figure}[H]
	...
	\label{fig:hamdaidien}
		    \begin{flushleft}
		\it\footnotesize Chú thích: Hàm mật độ đại diện thể hiện bằng đường thẳng in đậm cho ba hàm Gauss minh họa bằng nét đứt (a) dựa vào trọng số phân bổ đều giữa ba hàm $\mu_{ij}=1/3$, và (b) dựa vào vị trí tối ưu với khoảng cách $\mathcal{L}_2$. Nguồn: Minh họa của tác giả.
	\end{flushleft}
\end{figure}
...
\dng{[Ước lượng hàm mật độ bằng phương pháp hàm hạt nhân]
	Giả sử ta có một tập dữ liệu rời rạc gồm $n$ quan sát ${\bm{x}_j}$, với $j = 1\,; \ldots\,;  n$ trong không gian $\mathbb{R}^d$. Mỗi điểm $\bm{x}_j = (x_{1j}\,;  x_{2j}\,;  \ldots\,;  x_{dj})^\intercal$. Khi đó, hàm mật độ xác suất $\widehat f(\bm{x})$ tại điểm $\bm{x} = (x_1\,;  x_2\,;  \ldots\,;  x_d)^\intercal$ được ước lượng bằng phương pháp hạt nhân bởi công thức
	$$
...
\begin{table}[H]
	...
	\end{tabular*}
\begin{flushleft}
	\it\footnotesize Trong khoảng cách Chernoff, $0\le r\le 1$, và trong khoảng cách Matusita, $r\ge2$. Nguồn: Tác giả tổng hợp
\end{flushleft}
\end{table}
...
\caption{Bảng tổng kết các loại liên kết $\Delta$ giữa hai chùm hàm mật độ xác suất $\mathcal{F}$ và $\mathcal{G}$}\label{tab:LINKAGE}
	...
	\begin{tabular*}{\columnwidth}{@{\extracolsep\fill}lll@{\extracolsep\fill}} 
		...
		\end{tabular*}
\begin{flushleft}
 {\footnotesize \textit{Chú thích: \footnotemark[1]: tổng khoảng cách nhỏ nhất; \footnotemark[2]: tổng khoảng cách lớn nhất. Nguồn: Tác giả tổng hợp}}
\end{flushleft}
\end{table}
...
$ \int_{\Omega}f(x)\mathrm{\,d}x = \lim_{h\to0}\sum_{i}h^d w_if(x_i)$. Khi này, giả sử hàm $f\,:\,[a\,;b]\to \mathbb{R}$, ta chia đoạn $[a\,;b]$ thành $n$ phần bằng nhau với độ chia $h=(b-a)/n$, phương pháp hình thang một chiều xấp xỉ tích phân với mọi $x_i = a + ih$, và $i=0\,;\dots\,;n$,
	$$
...
Phân tích chùm phân vùng giả sử $\mathcal{S}$ là tập các véc-tơ $d$-chiều $x_j$, trong đó $j=1\,;2\,;\ldots\,;n$. Mỗi đối tượng $o_j$ liên hệ với hàm mật độ xác suất $f_j(x)$. Tập $\mathcal{F}$ gồm họ $n$-hàm mật độ trên $\Omega$. Bài toán phân tích chùm phân bổ $j$-hàm mật độ vào $c$-chùm ($c\in\mathbb{N}$), với $i=1\,;2\,;\ldots\,;c$ thỏa mãn ba ràng buộc
\begin{enumerate}
...
$
\mu_{ij} = \mathbf{1}\{i = \arg\min_s D^2(f_j, \theta_s)\}.
$
}
...
\vdu{[Phân tích chùm cứng hàm Gauss $1$-chiều]\label{vdu:KCF}
	Cho bảy hàm mật độ chuẩn $1$-chiều $\mathcal{F}$ cho bởi Tai Vo-Van \& Pham-Gia, T., (2010) \cite{Tai2010} có phương sai đồng nhất $\sigma_i^2 = 1$, với $i = 1\,;\ldots\,;7$, và các trung bình lần lượt là $\mu_i = \{0{,}3\,;4{,}0\,;9{,}1\,;1{,}0\,;5{,}5\,;8{,}0\,;4{,}8\}.$ Phân tích chùm KCF với $c=3$, và khoảng cách xét là $\mathcal{L}_2$.
	...
	\begin{figure}[H]
		...
		\begin{flushleft}
			\footnotesize\it Chú thích: Hình (a) trọng tâm chùm bằng nét liền và dữ liệu hàm mật độ nét đứt, và (b) ma trận phân vùng cứng của KCF. Nguồn: Minh họa của tác giả.
		\end{flushleft}
	\end{figure}
...
\dly{\label{thanhlapMU_Theta}Cho $\mathcal{F}$ chứa $n>c$ hàm mật độ xác suất, và $m>1$. Xét $D^2_{ij} = D^2(\theta_i\,;f_j)$ với $ 1 \le j \le n$ và $1 \le i \le c$. Với mỗi $j$, xác định các tập $I_j = \{i\,|\,D^2_{ij} = 0\,; 1 \le i \le c\}$ và tập $I^c_j = \{1\,;\ldots\,;c\} \setminus I_j$. Khi đó hàm mục tiêu \eqref{Jf} có thể đạt cực tiểu toàn cục khi và chỉ khi
\begin{align}
...
\begin{figure}[H]
	...
	\label{fig:phanvung}
		\begin{flushleft}
		\footnotesize \it Chú thích: Giá trị $\mu_{1j}$ tại trục tung biểu diễn mức độ phân vùng tại chùm thứ $1$ theo khoảng cách $ D^2_{1j} = \mathcal{L}^2_1(f_1\,;f_j)$ với các giá trị $m=\{1{,}1; 1{,}5; 2{,}0; 3{,}0; 5{,}0; 10{,}0\}$. Nét đậm hơn thể hiện $m=2$.
...
\begin{table}[H]
	...
	\textit{Chú thích}: $n$ là số lượng đối tượng trong dữ liệu hàm mật độ, $c$ là số lượng chùm, $d$ là số chiều dữ liệu, $i$ là tổng số vòng lặp.
	\end{flushleft}
...
$
\mathcal{D} = \{(f_j\,; y_j)\}$ với $j=1\,;\ldots\,;n$, trong đó, $f_j \in \mathcal{F}$ và $y_j \in \{-1\,; 1\}$.
...
Cho tập các hàm mật độ xác suất $\mathcal{F}$, trong đó mỗi $f_j$ là một hàm mật độ xác suất với $j=1\,;\ldots\,;n$. Cho trước $c$ là số chùm với $i=1\,;\ldots\,;c$. Gọi $\bm U=[\mu_{ij}]\in\mathbb{R}^{n\times c}$ là ma trận phân vùng, và $\Theta=\{\theta_1\,;\dots\,;\theta_c\}$ là các trọng tâm chùm. Giả sử tham số mờ $m>1$. Hàm mục tiêu của IFCF là tổng của hai phần bao gồm trọng số $1/\omega_i$ giảm ``hiệu ứng đồng nhất''.
\begin{align}
...
\end{align}
trong đó $\omega_i^{(t)}=\frac{1}{n}\sum_{j=1}^n \mu^{(t-1)}_{ij}$ là tỷ lệ kích thước mờ của chùm thứ $i$ với $i=1\,;\ldots\,;c$, thỏa mãn $\sum_{i=1}^c\omega_i=1$ với $\mu^{(t-1)}_{ij}$ là ma trận phân vùng tại vòng lặp thứ $(t-1)$.
...
\chm{Do ràng buộc phân hoạch mờ là độc lập theo từng mẫu $j$, và mục tiêu \eqref{eq:IFCF} là tổng theo $j$, nên bài toán tối ưu hoá theo $\bm U$ tách thành $n$ bài toán độc lập. 
	Cố định một $j\in\{1\,;\dots\,;n\}$, và đặt hằng số
	$
...
\bode{[Cập nhật trọng tâm $\Theta$ với $\mathcal{L}_2^2$]\label{lem2}
	Với $\bm U$ cố định, bài toán tối thiểu hoá $J_{IFCF}$ theo $\Theta$ là phân tích được theo từng $\theta_i$ với $i =1\,;\ldots\,;c$, và lồi nghiêm ngặt. Tồn tại duy nhất nghiệm cực tiểu $\Theta^*=G(\bm U)$ khi khoảng cách $D^2=\mathcal{L}_2^2$, 
	\begin{align}
		\theta_i(x)
		\;=\;
...
\begin{figure}[H]
	...
	\caption{Minh họa các công thức trọng tâm theo khoảng cách khác nhau đối với hàm mục tiêu \eqref{eq:IFCF}}\label{fig:trongtam_3KC}
		\begin{flushleft}
		\it\footnotesize Chú thích: Trọng tâm (nét liền) của ba hàm mật độ (nét đứt) qua các loại khoảng cách (a) $\mathcal{L}_2$, (b) Hellinger và (c) $2$-Wasserstein.
	\end{flushleft}
\end{figure}
...
\chm{
	Vì $T_{IFCF} = T_2\circ T_1$ là hợp của các hàm liên tục nên cũng liên tục. Ta chỉ cần chỉ ra $T_1$ và $T_2$ đều liên tục. Với $T_1 (\bm\mu, \Theta) = G(\bm\mu)$, $T_1$ liên tục nếu $G$ liên tục. Để chứng minh $G$ liên tục trong $cd$ biến $\bm\mu$, ta có $G$ là một trường véc-tơ, với ánh xạ $G\,:\,\mathbb{R}^{cd}\to \mathbb{R}^{cd}$. Vì $\{\bm\mu\mapsto \bm\mu^m\}$ là liên tục, $\{\bm\mu^m\mapsto \bm\mu^m/\omega_i\}$ là liên tục. Do đó, $G_{i}$ là thương của hai hàm liên tục nên liên tục với mọi $i$. Do đó, $G$ và $T_1$ liên tục trên toàn bộ tập xác định.
...
\dly{[Định lý hội tụ cho $J_{IFCF}$]\label{theo4}
	Xét tập $\mathcal{F}$-hàm mật độ $\mathcal{F} = \{f_1\,;\ldots\,;f_n\}$, cho hàm mục tiêu $J_{IFCF}(\bm U\,;\Theta)$ có dạng \eqref{eq:IFCF}, trong đó, ma trận phân vùng $\bm U$ thỏa mãn \eqref{c1_pro} và các trọng tâm chùm $\Theta=\{\theta_1\,;\theta_2\,;\ldots\,;\theta_c\}$. Nếu $T_{IFCF}\,:\,\mathbb{R}^{cd} \times \bm U\to \mathbb{R}^{cd} \times \bm U$ là một toán tử lặp Picard của $J_{IFCF}$, và với mọi $t$ sao cho $D^2(\theta_i^{(t)}\,;f_j)>0$, thì với mọi $(\bm\mu^{(0)}\,;\Theta^{(0)})\in \bm U\times [\mathrm{conv}(\mathcal{F})]^c$, hoặc
	\begin{enumerate}
...
\begin{figure}[H]
	...
	\begin{flushleft}
	\caption{Đánh giá thống kê các chỉ số cho Ví dụ~\ref{vdu:gauss1}}
	\label{fig:CD:vdu:gauss1}
			\footnotesize \it
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State \textbf{return} $c_j^*$ với $j=1\,;\ldots\,;n_{test}$
...
\begin{table}[H]
	...
	\end{tabular*}
	\begin{flushleft}\footnotesize\it
...
\bm D^{(test)}= \begin{bmatrix}
	0.490 & 0.494 & 0.001 & \cdots & 0.494 & 0.001\\
	0.002 & 0.005 & 0.495 & \cdots & 0.005 & 0.495\\
	0.494 & 0.491 & 0.516 & \cdots & 0.491 & 0.516
\end{bmatrix}_{3\times 20},
$$ và
$$
\bm U^{(test)}=  \begin{bmatrix}
	0.126 & 0.240 & 0.998 & \cdots & 0.240 & 0.998\\
	0.748 & 0.518 & 0.000 & \cdots & 0.518 & 0.000\\
	0.126 & 0.242 & 0.002 & \cdots & 0.242 & 0.002
\end{bmatrix}_{3\times 20}.
$$
...
Vì vậy, ta có thể tính được xác suất hậu nghiệm của các hàm mật độ kiểm tra với $\tau = \overline{D_{ij}}$ là $$\footnotesize\mathbb{P}(c=c_i\,;f=f^{(test)}_j)=\frac{\bm U_{ij} \exp(-\bm D_{ij}/\tau)}{\sum_s\bm U_{sj} \exp(-\bm D_{sj}/\tau)}= \begin{bmatrix}
	0.056 & 0.128 & 0.999 & \cdots & 0.128 & 0.999\\
	0.888 & 0.741 & 0.000 & \cdots & 0.741 & 0.000\\
	0.055 & 0.131 & 0.001 & \cdots & 0.131 & 0.001
\end{bmatrix}_{3\times 20}$$
...
Một hướng tiếp cận khác là phân chia ảnh thành các khối nhỏ có kích thước $s \times s$ và không chồng lấp từ phải sang trái, và từ trên xuống dưới. Mỗi ma trận vuông $P_k$ được ánh xạ thành véc-tơ
$
...
\vdu{\label{vdu:anh1}
	...
	\begin{figure}[H]
		...
		\end{subfigure}\hfill\\
			...
			\centering
			\includegraphics[width=\textwidth]{EX2_IR_10-eps-converted-to}
			\caption{}
		\end{subfigure}
		\begin{subfigure}[b]{.3\linewidth}
			\centering
			\includegraphics[width=\textwidth]{EX2_IR_50-eps-converted-to}
			\caption{}
		\end{subfigure}
				\begin{subfigure}[b]{.3\linewidth}
			\centering
			\includegraphics[width=\textwidth]{EX2_IR_100-eps-converted-to}
			\caption{}
		\end{subfigure}
...
Để phân đoạn hình ảnh này theo màu, trước tiên hình ảnh được chuyển đổi ảnh thang độ xám và quét cửa sổ trượt các mảng $(p\times p)$ không chồng lấn từ trái sang phải, từ trên xuống dưới. Mỗi mảng giờ được làm phẳng thành một véc-tơ $(p\times p)$-pixel, hàm mật độ cường độ của nó được ước tính thông qua hàm mật độ hạt nhân (Chương~\ref{chapter_1}, minh họa ở Hình~\ref{fig:kdepatchesdensity}), và khối lập phương $(\text{patch}\times h\times\text{pdf})$ kết quả trên $3$-D được lưu trữ để phân chùm các mảng. Ở đây, kích thước khối màu được đặt cố định là $4\times 4$.
