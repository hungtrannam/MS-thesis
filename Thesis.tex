...
\begin{figure}[H]\footnotesize
	...
	\caption[Các loại liên kết giữa ba chùm hàm mật độ xác suất]{Các loại liên kết giữa ba chùm hàm mật độ xác suất. }
	\label{fig:linkage}
\end{figure}
...
Thuật toán $k$-means nguyên bản được đề cập bởi J. Macqueen \cite{macqueen1967some} cho dữ liệu rời rạc nhưng mãi đến \cite{Tai2010,nguyen2024new}, nó được mở rộng cho hàm mật độ với ý tưởng giải bài toán bán giám sát. Xét $k$-means là bài toán tối tiểu đơn mục tiêu là tổng các bình phương trong chùm nhằm tìm kiếm các $\theta_i$, ta có
...
\end{align}
trong đó, $\mu_{ij}$ là biến tiềm ẩn thể hiện mức độ thành viên của hàm mật độ $j$ thỏa $\mu_{ij} =1$ nếu hàm $j$ thuộc chùm $c_k$ và $\mu_{ij}=0$ nếu ngược lại. Ký hiệu $\mathrm{d}^2(\theta_i\,,{f}_j)$ là khoảng cách giữa hàm mật độ trọng tâm đến các đối tượng $f_j$ còn lại trong dữ liệu.
...
\chy{
	So với kết quả phân tích chùm $k$-means, kết quả của phân tích chùm FCF mang tính tổng quát hơn. Dựa vào giá trị phân vùng $\mu{ij}$ , ta nhận thấy được mức độ tin cậy một đối tượng $f_j$ xếp vào chùm $c_i$. Nếu giá trị phân vùng $\max\mu{ij}$ lớn, ta có thể chắc chắn vào kết quả phân bổ, ngược lại, nếu giá trị này không cao, ta biết được hàm mật độ này có khả năng thuộc vào một chùm mới hơn là thuộc vào chùm đã cho.
}
...
\begin{table}[H]\small
	...
	\textit{Chú thích}: $n$ là \# đối tượng dữ liệu, $c$ là số chùm, $d$ là số chiều dữ liệu, $i$ là số vòng lặp.
	\end{flushleft}
