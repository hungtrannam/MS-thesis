...
\chm{Ta có
	\begin{align*}
		\mathbb{E}_{\theta_i}(X)
		= \int x\, \theta_i(x)\, \mathrm{d}x &= \int x\, \frac{\sum_{j=1}^n \mu_{ij} f_j(x)}{\sum_{j=1}^n \mu_{ij}}\, \mathrm{d}x = \frac{1}{\sum_{j=1}^n \mu_{ij}} \sum_{j=1}^n \mu_{ij}\underbrace{ \int x\, f_j(x)\, \mathrm{d}x}_{= \mathbb{E}_{f_j}(X)}.
	\end{align*}
...
\begin{align}\label{eq:log} \pi _i = {\mathbb {P}}(Y_i = 1 \vert {\mathcal {F}}(x) = {\mathcal {F}}_i(x)) =  \frac{\exp \left\{ \beta _0 + \int _{\Omega} {\mathcal {F}}(x) \beta (x) \mathrm{\,d}x \right\} }{1 + \exp \left\{ \beta _0 + \int _{\Omega} {\mathcal {F}}(x) \beta (x) \mathrm{\,d}x \right\} }, \end{align}
...
$$
l_i = \log\left(\frac{\pi_i}{1-\pi_i}\right) = \beta_0 + \int _{\Omega} {\mathcal {F}}(x) \beta (x) \mathrm{\,d}x.
$$
...
$$
l_i = \beta_0 + \sum_{m=1}^M \theta_m \underbrace{\int_\Omega \psi_m(x) {\mathcal{F}}_i(x)\, dx}_{=: \phi_{i,m}},
$$
trong đó $\phi_{i,m}$ là mô-men của hàm mật độ ${\mathcal{F}}_i(x)$ trên hàm cơ sở $\psi_m(x)$. Khi đó. bài toán ước lượng $\beta(x)$ chuyển thành
$$
\mathcal{L} = \max_{\beta_0, \theta} \sum_{i=1}^n \left[y_i \log \pi_i + (1 - y_i)\log (1 - \pi_i)\right]
$$
...
$\mu_\mathcal{F}=\{1,0;1,5; 2,0; 2,5; 6,0\}$ và $\mu_\mathcal{G}=\{4,5;7,5; 8,5; 8,0; 8,5\}$, có $\sigma_{\mathcal{F}}^2=0,4$ và$\sigma_{\mathcal{G}}^2=0,9$. Hãy phân loại hàm mật độ Gauss mới $\mathcal{N}(3,0;0,9)$ vào một trong hai lớp.
