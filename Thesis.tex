...
Để xuyên suốt trong quá trình thực hiện các thuật toán đề xuất, phần này sử dụng ánh xạ đo được $\mathcal{F}\,:\,(\Omega, \mathcal{S}, P) \to (\mathcal{D}\,,\mathcal{B}_D)$ với $(\Omega, \mathcal{S}, P)$ là không gian xác suất và  sigma-đại số Borel $\mathcal{B}_D$  trên $(\mathcal{D}, d)$. Một tập đối tượng hàm mật độ quan sát là $f_j\sim\mathcal{F}$ với $j=1\,,\ldots\,,n$ (Định nghĩa~\ref{dng:data1} và \ref{dng:datad}). 
...
\begin{figure}[H] 
	...
	\caption{Quan hệ giữa tỷ lệ mất cân bằng và quy mô cụm chiếm ưu thế}    \label{fig:dominant_ratio_IR}
...
\end{align*}
  ...
  &\stackrel{\delta=\frac{\#c_1}{\#c_2}}{=}\lim_{\#c_1 \to \infty}\left(\frac{\delta(u^{2m} - (1-u)^{2m})}{(\delta u^m + (1-u)^m)(\delta(1-u)^m + u^m)}\right)^2\int\left(\widehat\theta_1(x)-\widehat\theta_2(x)\right)^2 \mathrm{\,d}x
  \end{align*}
  
  Ta biết trong tỷ lệ trên, $\delta$ có bậc của tử nhỏ hơn bật của mẫu, nên dẫn đến giới hạn khoảng cách giữa hai trọng tâm khi số lượng phần tử chùm lớn tăng lên là
  $ \lim_{\#c_1 \to \infty} \mathcal{L}^2_2(\theta_1\,,\theta_2) = 0 $. Điều này chứng minh rằng FCF (và KCF) đều có xu hướng làm cho các trọng tâm chùm cuối cùng dịch chuyển gần nhau hơn so với các trọng tâm thực tế . Kết quả là, các chỉ số đánh giá phân chùm truyền thống sẽ ưu tiên gộp chùm thiểu số và chùm lân cận thành một chùm duy nhất.  Trong luận văn này, hiệu ứng đồng nhất lần đầu tiên được đề cập và giải quyết cho đối tượng hàm mật độ xác suất mà bỏ qua hiệu ứng xi-phông bởi do sự ưu tiên của nó.
...
Cho dữ liệu huấn luyện gồm $N$ hàm mật độ xác suất $f_j$ và $n$ hàm mật độ mới cần phân loại.  Giả sử $c$ lớp $c_i$, trong đó lớp $c_i$ chứa $n_j$ đối tượng hàm mật độ. Xác suất tiên nghiệm $q_{ij}$ (hay $u_i$) được xác định thông qua mối quan hệ mờ giữa hàm mật độ xác suất cần phân loại $h(x)$ và các lóp trong tập huấn luyện, sử dụng kỹ thuật phân tích chùm mở IFCF (Thuật toán~\ref{alg:IFCF}). Phân loại một hàm mật độ mới  $h(x)$ vào lớp $c_i$ bằng quy tắc Bayes nếu nó có xác suất hậu nghiệm đối với lớp đó là cao nhất, tức là $c_i = \arg \max_{1 \leq i \leq c} \left\{\mu_i \cdot D(h, \theta_i)\right\}.$ Trong đó, khoảng cách $D$ được chuẩn hóa trong khoảng $[0\,;1]$. Nguyên tắc này chỉ định rằng một hàm mật độ được gán vào một lớp cụ thể nếu nó đồng thời sở hữu giá trị cao nhất đối với xác suất tiên nghiệm và độ đo tương đồng đối với lớp đó (Thuật toán~\ref{alg:IFCF_QB}).
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\[
		c_j^*=\arg\max_{1\le i\le c}\{\,q_{ij}\,d_{ij}\,\}
		\]
...
\begin{table*}[h]
	...
	\begin{tabular*}{\textwidth}{@{\extracolsep\fill}lp{7cm}l@{\extracolsep\fill}}
		...
		FCF-$CWD$ \cite{vovan2019cluster} & dựa trên $c$-means mờ truyền thống $c$-means với khoảng cách độ rộng cụm giữa hai hàm mật độ & $maxIter = 500\,, m=2\,, \varepsilon = 1\times 10^{-6}$ \\
...
Theo kết quả phân vùng, các bảng của Hình~\ref{fig:U_ex3}(a)--Hình~\ref{fig:U_ex3}(d) hiển thị thành viên trên mỗi mẫu của từng thuật toán cụm (ngoại trừ FCM-$\mathcal {L}_1$) với $IR = 80$. Hình minh họa rằng chỉ có IFCF mới duy trì được độ sắc nét hoàn hảo, chứng minh một ngưỡng mất cân bằng chưa được ghi nhận, vượt quá ngưỡng này, các phương pháp dựa trên trọng tâm không thể duy trì các phân vùng đáng tin cậy.
...
Hơn nữa, để đánh giá xác thực phân vùng, Bảng~\ref{tab:EX3_EVA} cho thấy IFCF đạt tốt nhất, có khả năng chống nhiễu cao nhất và duy trì hiệu suất tính toán tốt.

Một mặt, FCM-CWD và FCM-$\mathcal {L}_1$ giống hệt nhau về mặt thống kê trên mọi phép đo, chứng tỏ rằng hiệu chỉnh khoảng cách không mang lại lợi ích đáng kể nào trong các môi trường mất cân bằng. Hơn nữa, KCF thể hiện sự sụt giảm chất lượng mạnh tại $IR\ge 80$, trong đó $ARI$ giảm mạnh từ $1,00$ xuống $0,02$ và Dunn giảm xuống $0,00$; Điều này cho thấy một ngưỡng nhiễu tới hạn, vượt quá ngưỡng này, các phương pháp trọng tâm cổ điển sẽ mất toàn bộ cấu trúc.
Hơn nữa, SUP đạt được Silhouette $=1$ nhưng đồng thời tạo ra các giá trị Dunn dao động từ $6112$ tại $IR=20$ xuống $141$ tại $IR=80$, cho thấy tính đồng nhất nội bộ của chùm tốt. Tuy nhiên, phương pháp này chỉ ổn định trong $4$-$5$ lần lặp, nhưng sự thời gian chạy tăng tuyến tính ($>49$s tại $IR=100$) cho thấy sự không phù hợp giữa số lần lặp và hiệu quả tính toán thực tế.
Mặt khác, IFCF là phương pháp đầu tiên được chứng minh là hoạt động đồng thời trên cả năm tiêu chí $ARI=NMI=1,0$, Chỉ số Dunn khoảng $6,0$, Chỉ số Silhouette khoảng $0,99$, thời gian chạy $<0,11$ giây và hội tụ trong $5-10$ lần lặp trên các thiết lập cho trước. Điều này chứng minh rằng khả năng chống mất cân bằng, tính nhỏ gọn và khả năng mở rộng.
...
Các đường cong trong Hình~\ref{fig:U_ex3} có cùng trục hoành $m\in\{1.1\,,1.5\,,2.0\,,2.5\,,5.0\,,10.0\}$ và trục tung là ARI và Silhouette. Trong các Hình~\ref{fig:U_ex3} chỉ có đường cong $IR=20$ cho kết quả ARI cao, trong khi các đường cong $IR > 50$ giảm gần $0,00$ ngay từ $m$ thấp.

Kết quả thực nghiệm cho thấy hiệu suất phân chùm phụ thuộc rõ rệt vào tham số mờ $m$ khi $IR$ thay đổi. Trong thuật toán đề xuất, phân chùm chất lượng cao được duy trì đối với các giá trị $m$ nhỏ, bất kể $IR$. Khi $m$ tăng, sự suy giảm hiệu suất xảy ra đột ngột hơn đối với $IR$ cao hơn, tuy nhiên phương pháp đề xuất cho thấy sự suy giảm chậm hơn đáng kể so với hành vi FCF thông thường được báo cáo trong các nghiên cứu trước đó \cite{nguyentrang2017fuzzy}. 

Cụ thể, đối với $IR=20$ hoặc $50$, cả $ARI$ và Silhouette vẫn gần tối ưu cho đến khi $m=5$, sau đó điểm số giảm mạnh. Đối với $IR> 80$ cao hơn, phân chùm mờ thông thường, như được ghi lại trong các công trình trước đó \cite{nguyen2024new,nguyen2024efficient}, có xu hướng mất khả năng phân tách chùm ở $m=2$ nhỏ hơn nhiều, thường dẫn đến các phép gán gần như ngẫu nhiên. Ngược lại, phương pháp đề xuất làm chậm sự sụp đổ này, duy trì chất lượng phân chùm cạnh tranh trên phạm vi $m$ rộng hơn.


\subsubsection{b.\qquad Ví dụ~\ref{vdu:anh2}}

Hình~\ref{fig:APP} cho thấy kết quả phân tích chùm BKIFF với số chùm $c$ tăng dần từ $2-10$ được áp dụng để phân đoạn ảnh Landsat của đảo Yam và vùng biển xung quanh. Hình cho thấy mức độ chi tiết của việc tách lớp phủ đất tăng lên theo số chùm $c$.
...
Kết quả là, trên $c=2\div10$, quá trình phân đoạn BKIFF diễn ra đơn điệu mà không tạo ra các hiện tượng nhiễu hạt muối tiêu.
Ở $c=2$, thuật toán phân tách cảnh thành vùng nước sâu và các vùng ven biển xung quanh. Khi chùm $c$ tăng dần về phía $3$, độ tương phản quang phổ giữa các rạn san hô và hòn đảo trở nên rõ ràng hơn, cho thấy các đường viền rõ nét hơn. Giữa $c = 4$ và $c = 6$, hòn đảo trở nên rõ nét hơn, thể hiện các khu vực cát nhẹ, thảm thực vật và các vùng biển khác nhau. Ngoài ra, một lớp mới được đưa vào để làm nổi bật sự khác biệt trong nước biển. Tiếp theo, các lớp vi mô $c = 7\div9$, các đặc điểm như bãi cát mỏng, chiếm chưa đến $0,5$\% tổng số pixel, vẫn được nhóm lại, thể hiện khả năng của thuật toán trong việc bảo tồn các dấu hiệu cường độ sáng nhỏ. Cuối cùng, ở $c = 10$, không có chùm nào bị phá vỡ; thể hiện tính nhất quán không gian nội tại của thuật toán.

Tóm lại, số chùm trung bình $c$ từ $4\div6$ là phù hợp nhất để cân bằng khả năng phân biệt lớp phủ và khả năng khái quát hóa đối với Ví dụ này. Ngược lại, các giá trị $ c $ cao hơn phù hợp hơn với các nghiên cứu vi mô đòi hỏi phải điều tra sâu hơn và phân tích chi tiết.
...
Nhận dạng thống kê cho hàm mật độ xác suất là một chủ đề mới vì cho đến hiện tại các lý thuyết và ứng dụng của nó đều cần nhiều nỗ lực xây dựng. Một số điểm tổng kết trong cả ba chương bao gồm
...
Trong quá trình lặp, giả sử hai trọng tâm $\Theta$ được xác định, khi đó với bất kỳ hàm mật độ $j$ thuộc chùm $c_1$, các mức độ phân vùng được cập nhật theo Công thức~\ref{eq:ifcf_u_final}, và điều kiện $\sum_{j=1}^c u_{ij}=1$, có thể được viết lại thành
\begin{align*}
...
Do đó, với chùm lớn $c_1$, giả sử tồn tại một sai số nhỏ $\varepsilon > 0$ sao cho 
$\omega_1 = \omega_{1,\text{true}} + \varepsilon$, trong đó $\omega_{1,\text{true}}$ là tỷ lệ thực của kích thước hàm mật độ thực tế $c_1$ so với toàn bộ tập dữ liệu.
