...
Đây là khái niệm quan trọng trong luận văn này. Để cụ thể, chúng tôi tập trung khảo sát tính chất của hàm mật độ ($\mu$ là độ đo Lebesgue) liên tục tuyệt đối của biến ngẫu nhiên $X$. Các hàm mật độ cũng được định nghĩa trên tập số thực. Ta có thể tổng quát hóa đối tượng hàm mật độ thành không gian lồi $\mathcal{F} = \left\{f\,:\,\Omega\to \mathbb{R}^+\vert \int_\Omega f(x)\mathrm{\,d}\mu(x)=1\right\}.$
...
\chm{Thật vậy, các giá trị trọng số $0\le\mu_{ij}\le1$ nên các $\theta_i$ liên tục không âm, và 
...
Theo Định nghĩa \ref{D}, khoảng cách giữa hai hàm mật độ thỏa mãn các tiên đề của mê-tríc. Trong lý thuyết thống kê và thông tin, một mê-tríc được gọi là \emph{khoảng cách} và nửa mê-tríc gọi là \emph{phân kỳ}. Các khoảng cách của hai hàm mật độ xác suất được thành lập theo hai các tiếp cận: véc-tơ và xác suất. Cách tiếp cận véc-tơ xem xét các mức giá trị là điểm trong không gian Euclide, nên chúng cũng có thể tính toán dựa vào các khoảng cách hình học thông thường. Bảng~\ref{tab1:KC_PDF} làm rõ các công thức khoảng cách và độ đo tương tự giữa hai hàm mật độ xác suất.
...
\chy{Nhìn vào Bảng \ref{tab1:KC_PDF}, có thể thấy khoảng cách Chernoff là một tổng quát hóa của khoảng cách Bhattacharyya khi chọn $s = 1/2$. Đáng chú ý, cả hai loại khoảng cách $\mathcal{L}_p$ và Hellinger đều thỏa mãn đầy đủ các tính chất mê-tríc trong không gian các hàm mật độ xác suất $\mathcal{F}$.}
...
Ở Hình~\ref{fig:dayKC}, chúng tôi ghi nhận giá trị khoảng cách $f_1\sim\mathcal{N}(0\,,1)$ và $f_2\sim \mathcal{N}(\mu\,,1)$ với $\mu$ trượt từ $-10$ đến $20$. Ta nhận ra khoảng cách $\mathcal{L}_1$ là hàm không trơn tại $\mu = 0$, tức $f_1$ trùng với $f_2$. Đồng thời, ta trực quan tính bị chặn của các loại khoảng cách này đúng với lý thuyết tại Bảng~\ref{tab1:KC_PDF}.
...
Phân tích chùm là một nhánh chính của nhận dạng không được giám sát \cite{jain2010data,saxena2017review,dinh2025categorical}. Nó có nhiệm vụ khám phá dựa vào sự tương tự giữa các mẫu trong dữ liệu đầu vào \cite{Bishop:DeepLearning24,webb2003statistical}. Ví dụ như khi ta cần khoanh vùng khối u trong hình ảnh MRI,  hay trong các báo cáo định tính cần nhóm/tổng hợp ý kiến hay quan điểm theo các chủ đề khác nhau. 
% Ta cũng có phân tích chùm các hàm mật độ xác suất là bài toán phân tích chùm cho đối tượng các hàm mật độ xác suất 

Thuật toán phân chùm dựa vào trọng tâm đề xuất hàm mục tiêu khác nhau, nhưng mục tiêu chung là sự phân bổ tối tiểu khoảng cách phần tử trong chùm trong khi vẫn giữ tối đa khoảng cách phần tử giữa các chùm. Các thuật toán này được chia thành hai loại dựa theo lý thuyết mờ \cite{bezdek2013pattern} là phân chùm không mờ (bao gồm $k$-mean \cite{nguyen2024new}, phân cấp thứ bậc và phân chùm tự cập nhật) và phân chùm mờ (bao gồm Fuzzy $c$-means \cite{nguyentrang2017fuzzy,vovan2019cluster}).
...
%Phân tích chùm phân vùng $k$-means và phân tích chùm tự cập nhật là các thuật toán đã được sử dụng tích cực cho dữ liệu hàm mật độ xác suất. Ngoài ra, chúng tôi bổ sung thêm một số kiến thức về phân cấp thứ bậc cũng đã được quan tâm trong lĩnh vực này.
...
\vdu{[Phân cấp thứ bậc hàm Gauss $1$-chiều]\label{VD:thubac} Cho bảy hàm mật độ chuẩn một chiều $\mathcal{F}$ cho bởi Vo-Van, Tai and Pham-Gia, T., \cite{Tai2010} có phương sai đồng nhất $\sigma_i^2 = 1\,, i = 1\,,\ldots\,,7$ và các trung bình lần lượt là $\mu_i = \{0,3\,;4,0\,;9,1\,;1,0\,;5,5\,;8,0\,;4,8\}.$ Phân cấp thứ bậc với khoảng cách $D(f_1\,,f_2)=\mathcal{L}_2$ và liên kết $\Delta_{\max}(\mathcal{F}_1\,,\mathcal{F}_2)$ là liên kết đủ.

Ta có kết quả phân cấp thứ bậc tích tụ bởi Hình~\ref{figs:thubac}. Bài giải chi tiết được phân tích trong Phụ lục B~(tr. \pageref{phulucB}).  Quá trình phân cấp được thực hiện với hai phương pháp liên kết đủ liên kết Ward. Nhìn Hình~\ref{figs:thubac}, ta thấy phương pháp liên kết đủ có xu hướng tạo ra các chùm nhỏ, chặt chẽ, và nhạy cảm với nhiễu ở biên. Ngược lại, liên kết Ward cho kết quả các chùm có tính đồng nhất cao hơn về phân bố, làm cây phân cấp cân đối.
	\begin{figure}[H]
	     \centering
...
\vdu{[Phân chùm mờ hàm Gauss $1$-chiều]\label{VD:FCF} Ta sử dụng lại dãy hàm mật độ trong Ví dụ~\ref{VD:thubac}. Phân tích $\mathcal{F}$ thành ba chùm, biết $m=2$ và khoảng cách là $\mathcal{L}_2$.
...
Ngược lại, phân tích chùm thứ bậc không đòi hỏi tiên nghiệm về số cụm, nhưng chi phí tính toán cao, đặc biệt với dữ liệu lớn khi cần tính toàn bộ ma trận khoảng cách
}

Tóm lại, các thuật toán phân tích chùm mang lại tiềm năng lớn trong việc khám phá các cấu trúc ẩn và những mối quan hệ có ý nghĩa giữa các mẫu dữ liệu, từ đó hỗ trợ các phân tích chuyên sâu và mở ra nhiều ứng dụng tiềm năng \cite{nguyen2024new,nguyentrang2017fuzzy}. Mặc dù kết quả của các thuật toán phân tích chùm có thể chịu ảnh hưởng bởi dữ liệu đầu vào, thể loại tự nhiên và lựa chọn thuật toán, chúng vẫn đóng vai trò là công cụ mạnh mẽ giúp người dùng nhận diện và diễn giải các mẫu tốt hơn. Qua đó, câu hỏi đặt ra rằng \emph{các loại tự nhiên có là những gì thuật toán phân cụm phần nào hướng đến trong điều kiện lý tưởng không} là một chủ đề nghiên cứu đầy hứa hẹn, tiếp tục được quan tâm và mở rộng trong cộng đồng khoa học thống kê \cite{leonelli2019data,watson2023philosophy,watson2023reply,mollema2024responding}.
...
\begin{align}\label{eq:log} \pi _j = {\mathbb {P}}(Y_j = 1 \vert {\mathcal {F}}(x) = {\mathcal {F}}_j(x)) =  \frac{\exp \left\{ \beta _0 + \int _{\Omega} {\mathcal {F}}(x) \beta (x) \mathrm{\,d}x \right\} }{1 + \exp \left\{ \beta _0 + \int _{\Omega} {\mathcal {F}}(x) \beta (x) \mathrm{\,d}x \right\} }, \end{align}
...
$$
\ell_j = \log\left(\frac{\pi_j}{1-\pi_j}\right) = \beta_0 + \int _{\Omega} {\mathcal {F}}(x) \beta (x) \mathrm{\,d}x.
$$
...
$$
\ell_j = \beta_0 + \sum_{m=1}^M \theta_m \underbrace{\int_\Omega \psi_m(x) {\mathcal{F}}_i(x)\, dx}_{=: \phi_{j,m}},
$$
trong đó $\phi_{j,m}$ là mô-men của hàm mật độ ${\mathcal{F}}_j(x)$ trên hàm cơ sở $\psi_m(x)$. 

Bài toán ước lượng $\beta(x)$ chuyển thành cực đại hóa log-likelihood của hồi quy logistics
$$
\mathcal{L} = \max_{\beta_0, \beta(\cdot)}\left\{\sum_{j=1}^n \left[y_j \log \pi_j + (1 - y_j)\log (1 - \pi_j)\right] - \underbrace{\lambda_1\sum_{m=1}^M\vert\beta_m\vert }_{Lasso} \underbrace{-\frac{\lambda_2}{2}\sum_{m=1}^M\beta_m^2}_{Ridge}\right\},
$$ 
trong đó, $\pi _j = {\mathbb {P}}(Y_j = 1 \vert {\mathcal {F}}(x))$. 




\vdu{[Phân loại hàm mật độ bằng hồi quy logistic]\label{VD:log}
	Xét hai lớp hàm mật độ Gauss $\mathcal{F}$ và $\mathcal{G}$ với các trung tâm lần lượt:
	\[
	\mu_\mathcal{F}=\{1,0;\,1,5;\,2,0;\,2,5;\,6,0\}, \quad
	\mu_\mathcal{G}=\{4,5;\,7,5;\,8,5;\,8,0;\,8,5\},
	\]
...
Trong bài toán phân loại Bayes, xác suất tiên nghiệm $\mathbb{P}(c_k)$ thường được ước lượng bằng nhiều cách khác nhau, thông thường là dựa vào kiến thức chuyên gia hoặc thông tin trong lịch sử nghiên cứu tương tự hoặc dữ liệu lớn. Trong trường hợp ta không có thông tin tiên nghiệm nào, ta có thể giả định mọi lớp đều có xác suất bằng nhau, tức là $\mathbb{P}(c_k)=1/k$. Trong nhiều tình huống, xác suất tiên nghiệm mà còn có thể ước lượng từ tần suất xuất hiện của các lớp trong tập dữ liệu huấn luyện. Khi đó, $\mathbb{P}(c_k)=n_k/n$  trong đó $n_k$là số lượng mẫu thuộc lớp $c_k$ và $n$ là tổng số mẫu. Tuy nhiên, phương pháp này có thể gặp vấn đề khi dữ liệu ít hoặc có xác suất bằng 0 cho một số lớp, phương pháp Laplace được sử dụng để điều chỉnh, $\mathbb{P}(c_k) = (n_k+\alpha)/(n+\alpha k)$, trong đó $\alpha>0$ và $k$ là số lớp.
...
Trong ngữ cảnh dữ liệu dạng hàm mật độ xác suất, mất cân bằng không chỉ đến từ số lượng phân phối mẫu không đồng đều mà còn từ sự khác biệt về dạng phân phối, độ phân tán và độ cao của các đỉnh hàm. Điều này đặt ra nhu cầu phát triển các phương pháp phân loại đặc thù, có khả năng cân nhắc cả hình dạng và độ chênh lệch giữa các hàm mật độ trong quá trình học và đánh giá.
...
\begin{figure}[H]
	\centering
		...
		\centering
		\includegraphics[width=.8\linewidth]{C2_k_best_FCF.pdf}
		\caption{Liên kết Ward}
	\end{subfigure}
	     \begin{subfigure}[b]{.45\textwidth}
	\centering
	\includegraphics[width=\linewidth]{C2_pdf_tree.pdf}
	\caption{Liên kết Ward}
\end{subfigure}
	\begin{flushleft}
		\footnotesize\it
...
$ARI = \frac{RI - \mathbb{E}(RI)}{\max(RI) - \mathbb{E}(RI)},$
	...
	\]
trong đó, với\( p_i = \frac{\#\mathcal{F}_i}{n},\, p_j = \frac{\#\mathcal{G}_j}{n},\, p_{ij} = \frac{\#(\mathcal{F}_i \cap \mathcal{G}_j)}{n}, \) ta có, $I(\mathcal{P},\mathcal{Q}) = \sum_{i,j} p_{ij} \log \frac{p_{ij}}{p_i p_j}$, với $H(\mathcal{P}) = - \sum_i p_i \log p_i \,,
...
\begin{array}{c|cc}
	...
	\midrule
	\text{Thực tế dương tính} & \#TP & \#FN \\
	\text{Thực tế âm tính} & \#FP & \#TN \\
	\bottomrule
\end{array}
\]
...
\begin{table}[H]
	...
	Độ nhạy	(PRE) & $\dfrac{\#TP}{\#TP + \#FP}$                                                                 & Tỷ lệ mẫu được dự đoán Positive là đúng      &     $[0,1]$      \\
	Độ chuyên, TPR (REC)    & $\dfrac{\#TP}{\#TP + \#FN}$                                                                 & Tỷ lệ nhận diện đúng Positive                &     $[0,1]$      \\
...
Do vậy, để đánh giá mô hình một cách đầy đủ và tin cậy, cần kết hợp nhiều thước đo. Quan trọng hơn, việc tối ưu hiệu năng đi kèm với chú trọng giải thích lý do dự đoán \cite{erasmus2021interpretability,herm2023stop}, thông qua XAI\index{giải thích-AI (XAI)}, tương tác người dùng và các tiêu chuẩn đánh giá phù hợp, sẽ nâng cao độ tin cậy và giá trị ứng dụng thực tiễn của mô hình.
