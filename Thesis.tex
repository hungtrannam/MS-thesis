...
\dng{[Ước lượng hàm mật độ bằng phương pháp hàm hạt nhân]
	Giả sử ta có một tập dữ liệu rời rạc gồm $n$ quan sát ${\bm{x}_j}$ với $j = 1, \ldots, n$ trong không gian $\mathbb{R}^d$. Mỗi điểm $\bm{x}_j = (x_{1j}, x_{2j}, \ldots, x_{dj})^\intercal$. Hàm mật độ xác suất tại điểm $\bm{x} = (x_1, x_2, \ldots, x_d)^\top$ được ước lượng bằng phương pháp hạt nhân bởi
	\[
	\hat{f}(\bm{x}) = \frac{1}{n \prod_{i=1}^d {bw}_i} \sum_{j=1}^{n} \prod_{i=1}^{d} \kappa_i\left( \frac{x_i - x_{ij}}{{bw}_i} \right),
	\]
	trong đó $\kappa_i$ là hàm hạt nhân theo chiều $i$ và ${bw}_i > 0$ là tham số trơn ứng với chiều đó.
}
...
\caption{Bảng tổng kết các loại liên kết $\Delta$ giữa hai chùm hàm mật độ xác suất}
	...
	\begin{tabular*}{\columnwidth}{@{\extracolsep\fill}lll@{\extracolsep\fill}} 
			...
			Liên kết Ward \cite{ward1963hierarchical}&$\Delta_{Ward}$ &$\Delta_{Ward}(\mathcal{F},\, \mathcal{G}) = \frac{\#\mathcal{F} \cdot \#\mathcal{G}}{\#\mathcal{F} + \#\mathcal{G}} D^2\big(\theta_{\mathcal{F}},\, \theta_{\mathcal{G}}\big)
			$\\ \bottomrule
		\end{tabular*}
...
Khi hai ta cần tính khoảng cách từ hợp hai chùm hàm $\mathcal{F}\cup\mathcal{G}$ đến một chùm hàm $\mathcal{H}$, ta cũng có thể sử dụng các loại liên kết trên. Trong thực tế, các liên kết $\Delta(\cdot\,,\cdot)$ được biến đổi thành
\begin{align*}
	...
	\Delta_{avg}(\mathcal{F} \cup \mathcal{G},\, \mathcal{H}) &= \frac{1}{\#(\mathcal{F}\cup\mathcal{G}) \cdot \#\mathcal{H}} \sum_{f \in \mathcal{F} \cup \mathcal{G},\, h \in \mathcal{H}} D(f,\, h), \\
	\Delta_{centroid}(\mathcal{F} \cup \mathcal{G},\, \mathcal{H}) &= D\big(\theta_{\mathcal{F} \cup \mathcal{G}},\, \theta_{\mathcal{H}}\big), \\
	\Delta_{Ward}(\mathcal{F} \cup \mathcal{G},\, \mathcal{H}) &= \frac{\#(\mathcal{F}\cup\mathcal{G})\cdot \#\mathcal{H}}{(\#\mathcal{F} \cup \#\mathcal{G}) \cup \#\mathcal{H}} D^2\big(\theta_{\mathcal{F} \cup \mathcal{G}},\, \theta_{\mathcal{H}}\big).
\end{align*}
...
\end{align}
trong đó, $\mu_{ij}$ là biến tiềm ẩn thể hiện mức độ thành viên của hàm mật độ $f_j(x)$ thỏa $\mu_{ij}= \bm 1_{j\in c_i}$. Tức là $\mu_{ij} =1$ nếu hàm $j$ thuộc chùm $c_k$ và $\mu_{ij}=0$ nếu ngược lại.  Hàm $\theta_i(x)$ là hàm mật độ trọng tâm của chùm thứ $c_i$. Ký hiệu $\mathrm{D}^2(\theta_i\,,{f}_j)$ là khoảng cách giữa hàm mật độ trọng tâm $\theta(x)$ đến các đối tượng $f_j(x)$ còn lại trong dữ liệu. 

\bode{[Thành lập hàm mật độ đại diện]
Giả sử $\mathcal{F}$-hàm mật độ được phân thành $c$-chùm qua các mức độ thành viên $\mu_{ij}$, với bình phương khoảng cách $\mathrm{D}^2(\theta_i, f_j)$. Khi đó, hàm mật độ đại diện $\theta_i^\ast(x)$ tối ưu trên mỗi chùm $c_i$ được cho bởi công thức
\[
...
\begin{algorithm}
	\caption{Giải thuật phân cấp thứ bậc tích tụ}\label{alg:HCF}
	\begin{algorithmic}[1]
		\State \textbf{Data:} Khoảng cách $D(\cdot\,,\cdot)$ của tất cả $\binom{n}{2} = \frac{n(n-1)}{2}$ các hàm mật độ theo từng cặp. Xem mỗi hàm $f$ như một chùm riêng tự nó.
		\State \textbf{Result:} Chùm thứ bậc $\mathcal{C}_i$.
		\For{$i = n, n - 1, \dotsc, 2$}
		\State (1) Xác định khoảng cách nhỏ nhất;
		\State (2) Gộp hai chùm này;
		\State (3) Tính khoảng cách mới giữa các chùm thứ $i - 1$ còn lại.
		\EndFor
	\end{algorithmic}
\end{algorithm}
...
\vdu{[Phân chùm mờ hàm Gauss $1$-chiều]\label{VD1} Ta sử dụng lại dãy hàm mật độ trong Ví dụ~\ref{VD:thubac}. Hãy phân tích $\mathcal{F}$ thành ba chùm, biết $m=2$ và khoảng cách là $\mathcal{L}_2$.

\textbf{Bài giải.} Với các thông số ban đầu $c=3$, $\epsilon=10^{-3}$, ta tiến hành thuật toán phân tích chùm mờ. Giả sử hàm mật độ xác suất trọng tâm $\theta_i(x)$ ở ba chùm lần lượt là $\theta_1{(x)} \sim \mathcal{N}(0\,;1)\,, \theta_2{(x)} \sim \mathcal{N}(2\,;1)$ và $\theta_3{(x)} \sim \mathcal{N}(5\,;1)$ và phân vùng ở mỗi hàm $f_j$ vào ba trọng tâm này là đều nhau, tức là $\bm U^{(0)} = 1/3 \times \texttt{ones}(3;7)$.
...
\[
f(h \,|\, \mathcal{F}) = \frac{1}{n_F} \sum_{i=1}^{n_F} \int_{\Omega} f_j(x) h(x) \mathrm{\,d}x.
\]
...
\vdu{[Phân loại hàm mật độ bằng phương pháp Bayes] Trở lại giả thuyết của Ví dụ~\ref{VD:log}, ta cần phân loại hàm mật độ Gauss mới $h(x)=\mathcal{N}(3,0;0,9)$ vào một trong hai lớp.
	...
	\begin{aligned}
		f(h \,|\, \mathcal{F}) 
		&= \frac{1}{n_F} \sum_{i=1}^{n_F} \int_{\Omega} f_i(x) h(x) \,\mathrm{d}x = \frac{1}{5} \sum_{i=1}^{5} \frac{1}{\sqrt{2\pi (0,9 + 0,4)}} \exp\left(-\frac{(3,0 - \mu_{\mathcal{F}_i})^2}{2(0,9 + 0,4)}\right) \\
	...
	\begin{aligned}
		f(h \,|\, \mathcal{G}) 
		&= \frac{1}{n_G} \sum_{i=1}^{n_G} \int_{\Omega} f_i(x) h(x) \,\mathrm{d}x 
...
Các mã Python được sử dụng trong luận văn này được lưu trữ tại \url{https://github.com/hungtrannam/probclust}. Chúng tôi chạy trên hệ điều hành Ubuntu 24.04.2 LTS, nhân Linux 6.11.0-19-generic, 11th Gen Intel$\copyright$ Core$^\text{TM}$ i5-11400H$\times$12, bộ nhớ 16.0 GiB.
...
\textbf{Bài giải Ví dụ~\ref{VD:thubac}} Đầu tiên, ta xem thứ tự của $\mu_i$ chính là thứ tự của dữ liệu hàm mật độ $f_i$, tính toán các khoảng cách từng đôi hàm mật độ bất kỳ bằng khoảng cách $\mathcal{L}_2(f_i\,,f_j)$ với $i\,,j = 1\,,\ldots\,,7$, ta nhận được ma trận khoảng cách
$$
\bm D^{(t=0)} =
...
Cuối cùng, ta chọn xếp chùm $\{f_2,f_5,f_7\}$  vào chùm $\{f_3,f_6\}$ dựa vào khoảng cách chính xác nhỏ nhất (4 chữ số thập phân). Vậy ta có chùm $\{f_2\,,f_5\,,f_3\,,f_6\}$. Ta xếp chùm còn lại vào chùm mới này, thuật toán kết thúc với cấu trúc như sau

\begin{verbatim}
--> root (7 samples): [1, 2, 3, 4, 5, 6, 7]
|   --> root0 (4 samples): [1, 4, 3, 6]
|   |   --> root00 (2 samples): [1, 4]
|   |   |   --> root000 (1 sample): [1]
|   |   |   --> root001 (1 sample): [4]
|   |   --> root01 (2 samples): [3, 6]
|   |   |   --> root010 (1 sample): [3]
|   |   |   --> root011 (1 sample): [6]
|   --> root1 (3 samples): [2, 5, 7]
|       --> root10 (1 sample): [2]
|       --> root11 (2 samples): [5, 7]
|       |   --> root110 (1 sample): [5]
|       |   --> root111 (1 sample): [7]
\end{verbatim}
