...
\chy{Ta thấy khi hai hàm mật độ cách nhau quá xa, trường hợp này là $|\mu|\gg0$, tất cả loại khoảng cách  hàm mật độ đều bão hòa và không đo được sự khác biệt. Xét ba hàm mật độ đều $f_1\sim \mathcal U[-3\,,2]\,, f_2\sim \mathcal U[-2\,,-1]$ và $f_3\sim \mathcal U[2\,,3]$. Lúc này, khoảng cách $D(f_1\,,f_3) = D(f_2\,,f_3) = D(f_1\,,f_2) $, chỉ ra hai hàm mật độ về mặt không gian rất xa nhau, nhưng khoảng cách không thể đo xa hơn nữa vì nó đã đạt đến ngưỡng lớn nhất. Đây là hạn chế không giữ được vị trí hình học đối với các khoảng cách hàm mật độ.
	}
...
Trong khoa học máy tính, các khoảng cách giữa các hàm mật độ xác suất thường chứa các tích phân xác định, vốn không thể tính chính xác bằng phương pháp thông thường. Do đó, các tích phân này cần xấp xỉ bằng phương pháp số. Luận văn này, chúng tôi chọn sử dụng phương pháp hình thang\index{phương pháp hình thang (trapezoidal method)} nhằm cải thiện độ chính xác so với tổng Riemann. Mặc dù Simpson hoặc Monte-Carlo có thể đạt độ chính xác cao hơn, nhưng chúng đòi hỏi chi phí tính toán và triển khai phức tạp hơn.

Một tích phân trên miền $\Omega \subset \mathbb{R}^d$ được xấp xỉ bằng công thức tổng quát
