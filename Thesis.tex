...
\section{Các định nghĩa và tính chất cơ bản}
...
\dng{[Hàm mật độ đại diện Gauss] Một PDF đại diện Gauss có dạng phân phối chuẩn sao cho tổng khoảng cách giữa chúng đến các PDF khác là nhỏ nhất \cite{frechet1948elements}, tức là
...
%Cho $f\,;g$ là hai hàm mật độ Radon-Nikodym tương ứng với độ đo trong không gian đo được $(\Omega\,;\mathcal{B})$ với $\mathcal{X}\subseteq \mathbb{\mathbb{R}^d$.
 ...
 \dng{[Khoảng cách $\mathcal L_p$]\label{D} Cho hai PDF $f_1\,, f_2$ trên $\mathbb R^d$. Khoảng cách $\mathcal{L}_p$ được định nghĩa là$$ \mathcal L_p(f_1\,;f_2)=\left(\int_{\mathbb R^d}|f_1(x)-f_2(x)|^p\,\mathrm{d}x\right)^{1/p}\,, 1\le p<\infty .$$
	...
	Theo Định nghĩa \ref{D}, khoảng cách giữa hai PDF thỏa mãn các tiên đề của mê-tríc. Trong lý thuyết thống kê và thông tin, một mê-tríc được gọi là \emph{khoảng cách} và nửa mê-tríc gọi là \emph{phân kỳ}. Thực tế, ta thường sử dụng $p=1$ hoặc $p=2$ cho các tính toán. Ta cũng có mối quan hệ giữa hai hàm mật độ đối mới trọng tâm bất kỳ trong mê-tríc. 
...
Hình~\ref{fig:dayKC} ghi nhận các giá trị khoảng cách giữa hàm mật độ $f_1\sim\mathcal{N}(0\,;1)$ và $f_2\sim \mathcal{N}(\mu\,;1)$, với $\mu$ trượt từ $-10$ đến $20$. Ta thấy khi hai hàm mật độ $f_2$ cách nhau quá xa hàm $f_1$, tức là $|\mu_2|\gg0$, tất cả loại khoảng cách  hàm mật độ đều bão hòa và không đo được sự khác biệt. Lúc này hai hàm mật độ về mặt không gian rất xa nhau, nhưng khoảng cách không thể đo xa hơn nữa vì nó đã đạt đến ngưỡng lớn nhất. Đây là hạn chế không giữ được vị trí hình học đối với các khoảng cách hàm mật độ.
...
\vdu{[So sánh các loại khoảng cách hàm mật độ]\label{vidu:SS_KS} Xét hai PDF-đều $f_1(x)$ và $f_2(x)$ tại các vị trí khác nhau trên trục $x\in\mathbb{R}$.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=\linewidth]{Figs/C1_Uniform.pdf}
		\caption{Các trường hợp trong khoảng cách giữa hai hai hàm mật độ đều $f_1(x)$ và $f_2(x)$ đối với Ví dụ \ref{vidu:SS_KS}.}\label{fig:daydeu}
				\begin{flushleft}
...
\begin{table}[H]
	\caption{Các kết quả khoảng cách giữa hai hàm mật độ đều trong bốn tình huống vị trí đối với Ví dụ~\ref{vidu:SS_KS}}
...
\subsubsection{a.\quad Liên kết chùm hàm mật độ}\label{subsub:lienket}
...
\dly{\label{thanhlapMU_Theta}Cho $\mathcal{F}$-PDF chứa $n>c$ hàm mật độ xác suất, và $m>1$. Xét $D^2_{ij} = D^2(\theta_i\,;f_j)$ với $ 1 \le j \le n$ và $1 \le i \le c$. Với mỗi $j$, xác định các tập $I_j = \{i\,|\,D^2_{ij} = 0\,; 1 \le i \le c\}$ và tập $I^c_j = \{1\,,2\,,ldots\,,c\} \setminus I_j$. Khi đó hàm mục tiêu \eqref{Jf} có thể đạt cực tiểu toàn cục khi và chỉ khi
\begin{align}
...
\begin{table}[H]
	...
	\begin{flushleft}
	\fontsize{11pt}{14pt}\selectfont\it
Chú thích:	$n$ là số lượng đối tượng trong dữ liệu hàm mật độ, $c$ là số lượng chùm, $d$ là số chiều dữ liệu, $i$ là tổng số vòng lặp.
	\end{flushleft}
...
Cho tập huấn luyện gồm các hàm mật độ và nhãn của từng hàm
$
...
Phương pháp Na\"{\i}ve  Bayes đã được nhiều nhà thống kê quan tâm cho dữ liệu hàm mật độ xác suất bởi \cite{ren2009naive}. 
Cho tập huấn luyện gồm các hàm mật độ và nhãn của từng hàm
$
\mathcal{D} = \{(f_j\,; y_j)\}$ với $j=1\,,2\,,\ldots\,,n$, trong đó, $f_j \in \mathcal{F}$ và $y_j \in \{-1\,; 1\}$.
...
\vdu{[Phân loại hàm mật độ bằng phương pháp Na\"{\i}ve Bayes\index{phương pháp Na\"{\i}ve Bayes (Na\"{\i}ve Bayes method)}]
...
\end{subfigure}
\caption{Minh họa các dữ liệu hàm mật độ được thu thập trong thực tế từ các nghiên cứu trước đây}\label{tab:DL_thucte}
\begin{flushleft}
	\fontsize{11pt}{14pt}\selectfont\it
Chú thích:	(a) Dữ liệu \texttt{Tuna\_Fork\_Length} của\cite{minami2024regression} biểu diễn phân bố chiều dài thân cá ở các vùng biển khác nhau, (b) Dữ liệu \texttt{BOLD} của \cite{petersen2016functional} là chuỗi tính hiệu fMRI được biểu diễn bởi hàm mật độ xác suất, (c) Dữ liệu \texttt{Blood\_dataset} trong \cite{irpino2015basic} gồm các phân bố xác suất thực nghiệm của ba chỉ số sức khỏe, và (d) Dữ liệu \texttt{CTU} của \cite{nguyentrang2017fuzzy} là dữ liệu hàm mật độ $2$-chiều của GPA và rèn luyện của các Khoa/Viện Đại học Cần Thơ.
\end{flushleft}
\end{figure}
...
Trong lĩnh vực này, đã có nhiều công trình nghiên cứu chuyên sâu liên quan. Bài toán phân tích chùm hàm mật độ lần đầu tiên được giới thiệu vào những năm 2010 \cite{vovan2019cluster,vovan2025classifying}, mở ra hướng tiếp cận mới cho việc xử lý dữ liệu phức tạp. Hơn nữa, song song với sự phát triển của các phần tử rời rạc và công cụ mô hình hóa thống kê, bài toán phân tích chùm hàm mật độ đã không ngừng được hoàn thiện và mở rộng phạm vi ứng dụng trong thực tế. Tổng quan tiến trình phát triển của các phương pháp phân tích chùm hàm mật độ trong giai đoạn năm năm được trình bày trong Bảng~\ref{tab:in_clustering_summary}, qua đó thể hiện xu hướng tiến hóa từ các phương pháp truyền thống sang các kỹ thuật thích nghi và lai ghép hiện đại.
...
\begin{table}
	...
	\caption{Tổng quan nghiên cứu phân tích chùm cho hàm mật độ xác suất giai đoạn từ năm 2019--2024}
	\begin{tabular*}{\textwidth}{@{\extracolsep\fill}lp{3cm}p{5.5cm}p{3cm}p{2cm}@{\extracolsep\fill}} 
		...
		2024 & Okano Ryo \& Imaizumi Masaaki \cite{okano2024wasserstein} & Phân chùm cứng cho dữ liệu hàm mật độ &$W^2_2$ &$\times$   \\
		\bottomrule
	\end{tabular*}
...
Mặc dù các mục tiêu khác nhau, điểm chung giữa chúng là sự mở rộng nền tảng của phân chùm truyền thống sang các dạng dữ liệu có cấu trúc cao hơn. Về mặt ứng dụng, thuật toán entropy-jackknife là phương pháp phân chùm cứng phù hợp cho dữ liệu hình ảnh, trong khi mô hình của Tavakkol \& Son lại thích hợp hơn cho dữ liệu hàm mật độ đa chiều và liên tục.
...
Ngược lại, các cột trong hình mô tả mức độ nhiễu khác nhau của dữ liệu, FCF cho thấy các hàm mật độ biên của chùm nhỏ hơn bị kéo lệch về phía chùm lớn khi phân bố dữ liệu trở nên rối loạn.
Trong phạm vi của luận văn này, “hiệu ứng đồng nhất” được xem là vấn đề trung tâm và được đề cập, phân tích cũng như giải quyết lần đầu tiên đối với đối tượng là các hàm mật độ xác suất, trong khi “hiệu ứng xi-phông” được bỏ qua do mức độ ưu tiên thấp hơn trong bối cảnh nghiên cứu hiện tại.
...
\begin{figure}
	...
	\end{subfigure}
	\caption{Ảnh hưởng của dữ liệu PDF Gauss mất cân bằng lên FCF ($m=2\,;c=2\,; D=\mathcal{L}^2_2$)}\label{fig:IR_STD}
\end{figure}
...
$G\,:\,\bm U \to \mathbb{R}^{cd} \,; G(\bm\mu)=\Theta$, trong đó, $\Theta = (\theta_1\,;\theta_2\,; \ldots\,;\theta_c)$ và các hàm mật độ trọng tâm $\theta_i$ tính bởi công thức \eqref{trongtamFCF}. 
...
Tổng thể, Bảng~\ref{tab:KCTT} và Hình~\ref{fig:trongtam_3KC} thể hiện sự khác biệt các khoảng cách $\mathcal{L}_2$, $D_H^2$ và $W^2_2$ trong xác định trọng tâm chùm gồm ba hàm mật độ xác suất, cho thấy hình dạng, vị trí, và phân tán của nó.
...
\dly{[Định lý hội tụ cho $J_{IFCF}$]\label{theo4}
	Xét tập $\mathcal{F}$-hàm mật độ $\mathcal{F} = \{f_1\,;f_2\,;\ldots\,;f_n\}$, cho hàm mục tiêu $J_{IFCF}(\bm U\,;\Theta)$ có dạng \eqref{eq:IFCF}, trong đó, ma trận phân vùng $\bm U$ thỏa mãn \eqref{c1_pro} và các trọng tâm chùm $\Theta=\{\theta_1\,;\theta_2\,;\ldots\,;\theta_c\}$. Nếu $T_{IFCF}\,:\,\mathbb{R}^{cd} \times \bm U\to \mathbb{R}^{cd} \times \bm U$ là một toán tử lặp Picard của $J_{IFCF}$, và với mọi $t$ sao cho $D^2(\theta_i^{(t)}\,;f_j)>0$, thì với mọi $(\bm\mu^{(0)}\,;\Theta^{(0)})\in \bm U\times [\mathrm{conv}(\mathcal{F})]^c$, hoặc
	\begin{enumerate}
...
Chỉ số $Silhouette$ $S = \mathtt{mean}(s(f_j))$ đo mức độ phù hợp của một hàm mật độ $f_j$ với chùm mà nó được gán nhãn so với chùm gần nhất khác, trong đó $s(f_j) = \frac{b(f_j) - a(f_j)}{\max \{ a(f_j)\,; b(f_j) \}}$ với $	a(f_j) = \frac{1}{\#c_i - 1} \sum_{\substack{f \in c_i \\ f \ne f_j}} D(f_j\,; f)	$ là khoảng cách trung bình giữa $f_j$ và tất cả các hàm mật độ khác trong chùm $c_i$ chứa nó, và $	b(f_j) = \min\limits_{j \ne i} \frac{1}{\#c_j} \sum_{f \in c_j} D(f_j\,; f)$ là khoảng cách trung bình giữa $f_j$ và các hàm mật độ trong chùm gần nhất $c_j$ với $j \ne i$.
...
Ta cũng có thể khảo sát và chọn số chùm tối ưu qua các CVI này. Để minh họa, Hình~\ref{fig:c1kbestfcf} cho giá trị các chỉ số CVI với số chùm $c=2\div7$ đối với dữ liệu ở Ví dụ~\ref{VD:thubac}. Kết quả cho thấy chỉ số $Silhouette$ đạt cực đại tại $c=3$, chỉ số $Dunn$ cao nhất tại $c=6$, và $DBI$ giảm dần. Như vậy, cả ba chỉ số đều gợi ý rằng mô hình phân chùm có hiệu quả cao nhất khi số chùm $c=3$, thể hiện sự tách biệt giữa các nhóm hàm mật độ.
...
Đánh giá bên ngoài được sử dụng khi có nhãn thực tế $\mathcal{P} = \{\mathcal{F}_1\,; \mathcal{F}_2\,;\ldots\,; \mathcal{F}_R\}$ của tập hàm mật độ $\mathcal{F} = \{f_1\,; f_2\,;\ldots\,; f_n\}$ và kết quả phân chùm $\mathcal{G} = \{\mathcal{G}_1\,; \mathcal{G}_2\,;\ldots\,; \mathcal{G}_K\}$.  
...
\begin{table}[H]
	\centering
	\footnotesize
	\caption{Tổng quan phương pháp phân loại cho hàm mật độ xác suất trong giai đoạn 2019-2024}
	...
	\begin{tabular*}{\textwidth}{@{\extracolsep\fill}lp{6cm}p{8cm}@{\extracolsep\fill}}
		...
		2023 & Pavlů \textit{et al.} \cite{pavluu2023classification} & Nhúng hàm mật độ vào không gian Bayes, triển khai logistic hàm mật độ \\
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		% --- Training phase ---
		\State $t \gets 0$
		\State Khởi tạo ngẫu nhiên ma trận phân vùng ${\bm \mu}^{(0)}$ và tập hàm mật độ trung tâm $\Theta^{(0)} = \{\theta_i^{(0)}\}_{i=1}^{c}$
		...
		$$
		\theta_i^{(t)}(x)
		=\frac{\sum_{j=1}^{n_{train}} (\mu_{ij}^{(t-1)})^m f^{train}_j(x)}
		{\sum_{j=1}^{n_{train}} (\mu_{ij}^{(t-1)})^m},
		\quad 1 \le i \le c
		$$
		...
		\Comment{Nhãn của các hàm mật độ kiểm tra}
		\EndProcedure
	\end{algorithmic}
\end{algorithm}
...
\vdu{[Hàm Gauss-lệch, ba lớp] \label{vidu2:lechgauss} Giả sử phân phối lệch-Gauss từ \url{http://azzalini.stat.unipd.it/SN/}. Hàm mật độ của phân phối lệch–Gauss được định nghĩa như sau.
	\begin{align*}
		f(x \,\vert \, \xi\,; \sigma\,; \lambda)=\frac{2}{\lambda}\phi\left(\frac{x-\xi}{\sigma}\right)\Phi\left(\lambda\frac{x-\xi}{\sigma}\right),
	\end{align*}
	trong khung nghiên cứu này, tham số vị trí $\xi_i$ được lấy mẫu từ các phân phối chuẩn $\mathcal{N}(0\,;0.2)$, $\mathcal{N}(2\,;0.2)$ và $\mathcal{N}(-2\,;0.2)$ tương ứng cho chùm, $1$, $2$ và $3$. Các tham số độ lệch-Gauss được đặt là $\sigma_i=[0\,; 10\,; -10]$, trong khi các tham số lệch được xác định là $\lambda_i=[1\,; 2\,; 2]$, nhằm tạo ra sự bất đối xứng giữa các lớp.  Các hàm mật độ của ba lớp này được minh họa trong Hình~\ref{fig:vidu2:lechgauss}(a), với tỷ lệ $IR=1:20:20$ tương ứng cho ba lớp, với lớp nhỏ nhất  (lớp 1) có $10$ hàm mật độ.
...
Luận văn thực hiện các bước trong Thuật toán~\ref{alg:IFCF_QB} một cách tuần tự. Thuật toán gồm hai phần bao gồm tìm xác suất tiên nghiệm và phân lớp Bayes. Thuật toán tìm xác suất tiên nghiệm là IFCF (Thuật toán~\ref{alg:IFCF}) với khoảng cách tiêu chuẩn $\mathcal{L}^2_2$ và các tham số liên quan bao gồm $m = 2{,}0$, $c=3$, và khởi tạo bằng $k$-means++. Sau đó, phân lớp Bayes xác định các lớp mà tập kiểm tra thuộc về.

\textbf{Bước 1. Tiền xử lý}. Trong bước này, luận văn chia dữ liệu thành hai tập hợp các hàm mật độ huấn luyện và kiểm tra. Tập huấn luyện có chức năng đào tạo các hàm mật độ để xác định tiên nghiệm cho tập kiểm tra. Tập kiểm tra được chia ngẫu nhiên như sau.
Chọn cố định số lượng phần tử trong tập kiểm tra là $n_1 = 2\,; n_2= 10\,; n_3 = 8$. Mỗi hàm mật độ $f^{(test)}_j$ với $j = 1\,;\div\,;20$ được gán vào tập kiểm tra là tổ hợp $C_{n}^{n_i}$ với $i=1\div 3$ để mỗi lớp đều có hàm mật độ kiểm tra. Tập huấn luyện là phần còn lại sau khi đã chọn ngẫu nhiên kiểm tra.  Hình~\ref{fig:vidu2:lechgauss} thể hiện các hàm mật độ kiểm tra chọn ngẫu nhiên từ dữ liệu. Ta có số lượng tập huấn luyện và kiểm tra như sau

\begin{table}[H]
	\centering
	\footnotesize
	\caption{Phân bố số lượng và tỷ lệ hàm mật độ trong tập huấn luyện và kiểm tra đối với Ví dụ~\ref{vidu2:lechgauss}.}
...
\textbf{Bước 2. Xác định tiên nghiệm}. Bước này đánh giá tiên nghiệm dựa vào phân tích chùm hàm mật độ IFCF trên tập huấn luyện.

\textit{Bước 2.1.} Khởi tạo tập hợp ba hàm mật độ trọng tâm $\Theta^{(t=0)}$ bằng $k$-means++\index{$k$-means++}. Ta có Hình~\ref{fig:vidu2:trongtam}(a) thể hiện ba hàm mật độ trọng tâm khởi tạo cho ba lớp lần lượt là $\Theta =\{f_{98}\,; f_{6}\,;f_{226}\}$. Khi không có thông tin gì về kích thước lớp, ta khởi tạo $\omega_i=1/c=1/3$ cho cả ba lớp. Tại bước lặp thứ nhất $t=1$, ta cập nhật các giá trị của $\bm U$ và $\Theta$.
...
\textit{Bước 2.2.} Ta tiến hành tính khoảng cách $\mathcal{L}_2(\theta_i\,;f_j)$ giữa các trọng tâm khởi tạo với tất cả các hàm mật độ xác suất còn lại. Ở đây, luận văn trình bày cụ thể cột đầu tiên như sau.
\begin{align*}
...
Vì vậy, ta có thể tính được xác suất hậu nghiệm của các hàm mật độ kiểm tra với $\tau = \overline{D_{ij}}$ là $$\footnotesize\mathbb{P}(c=c_i\,;f=f^{(test)}_j)=\frac{\bm U_{ij} \exp(-\bm D_{ij}/\tau)}{\sum_s\bm U_{sj} \exp(-\bm D_{sj}/\tau)}= \begin{bmatrix}
	0{,}06 & 0{,}13 & 0{,}10 & \cdots & 0{,}13 & 0{,}10\\
	0{,}89 & 0{,}74 & 0{,}00 & \cdots & 0{,}74 & 0{,}00\\
	0{,}06 & 0{,}13 & 0{,}00 & \cdots & 0{,}13 & 0{,}00
\end{bmatrix}_{3\times 20}$$
Từ đây ta biết được các hàm mật độ được xếp vào lớp $c_i$ là $\widehat y_j=\{2\,;2\,;\underbrace{1\,;1\,;\ldots\,;1}_\text{10 lần}\,;\underbrace{3\,;\ldots\,;3}_\text{8 lần}\} $. 
...
Các hàm mật độ cục bộ $\hat{f}_k(u)$ là kết quả sau khi ước lượng cho từng khối. Hình~\ref{fig:kdepatchesdensity} thể hiện bốn khối màu của hình ảnh mô phỏng (của Hình~\ref{fig:kdeimagedensity}) đánh theo thứ tự $1\div 4$ theo thứ tự tư trái sang phải, từ trên xuống dưới.
...
\begin{figure}[H]
	...
	\begin{subfigure}[h]{0.47\linewidth}
		...
		\caption{}
	\end{subfigure}
	\caption{Hình ảnh thực tế và các hàm mật độ xác suất trích xuất từ khối màu}
	\begin{flushleft}\fontsize{11pt}{14pt}\selectfont\it
	Chú thích:	(a) Hình ảnh Landsat đã chuyển về ảnh xám, và (b) Mật độ của các khối màu ($4\times4$)
	\end{flushleft}
\end{figure}

Để phân đoạn hình ảnh này theo màu, trước tiên hình ảnh được chuyển đổi ảnh thang độ xám và quét cửa sổ trượt các mảng $(p\times p)$ không chồng lấn từ trái sang phải, từ trên xuống dưới. Mỗi mảng giờ được làm phẳng thành một vector $(p\times p)$-pixel, hàm mật độ cường độ của nó được ước tính thông qua hàm mật độ hạt nhân (Chương~\ref{chapter_1}, minh họa ở Hình~\ref{fig:kdepatchesdensity}), và khối lập phương $(\text{patch}\times h\times\text{pdf})$ kết quả trên $3$-D được lưu trữ để phân chùm các mảng. Ở đây, kích thước khối màu được đặt cố định là $4\times 4$.
...
\begin{table*}[h]
	...
	\begin{tabular*}{\textwidth}{@{\extracolsep\fill}lp{7cm}l@{\extracolsep\fill}}
		...
		FCF-$\mathcal{L}_1$ \cite{nguyentrang2017fuzzy} & dựa trên $c$-means mờ truyền thống với $\mathcal{L}_1$ giữa hai hàm mật độ & $maxIter = 500\,; m=2\,; \varepsilon = 1\times 10^{-6}$ \\
		FCF-$CWD$ \cite{vovan2019cluster} & dựa trên $c$-means mờ truyền thống $c$-means với khoảng cách độ rộng chùm giữa hai hàm mật độ & $maxIter = 500\,; m=2\,; \varepsilon = 1\times 10^{-6}$ \\
		KCF \cite{nguyen2024new} &dựa trên $k$-means truyền thống với khoảng cách $\mathcal{L}_1$ giữa hai hàm mật độ & $maxIter = 500$ \\
		IFCF (Đề xuất) & dựa trên $c$-means mờ  cải tiến cho hàm mật độ mất cân bằng & $maxIter = 500\,; m = 2\,; \varepsilon = 1\times 10^{-6}$ \\
		\bottomrule
	\end{tabular*}
\end{table*}
...
Tập hợp các hàm mật độ $f_j(x)$ được nội suy trên lưới đều $\Omega = [0\,;1]$ gồm 256 điểm. Kết quả cuối cùng là một tập dữ liệu gồm $N=410$ hàm mật độ xác suất  với kích thước mỗi hàm $256$ điểm. Hình~\ref{fig:vidu:dat2}(b) minh họa ngẫu nhiên 10 hàm mật độ chia theo từng lớp đất. 
...
Kết quả trong Bảng~\ref{tab:bayes_ifcf_results} cho thấy sự khác biệt rõ rệt giữa các phương pháp phân loại hàm mật độ.
...
Về mặt lý thuyết, các hướng phát triển tiếp theo được định hình nhằm mở rộng và hoàn thiện khung phương pháp luận cho nhận dạng thống kê với đối tượng PDF.

\begin{enumerate}[label=(\roman*), leftmargin=2em, itemsep=1ex]
	\item {Phát triển các mô hình nhận dạng kết hợp} để đồng thời giải quyết hai hiệu ứng “đồng nhất” và “xi phông”. Thuật toán IFCF được đề xuất trong luận văn đã góp phần giảm thiểu “hiệu ứng đồng nhất”. Tuy nhiên, các thuật toán phân chùm vẫn phải đối mặt với “hiệu ứng xi phông”, trong đó các phần tử biên của lớp nhỏ có xu hướng bị kéo lệch về chùm lớn hơn.
	...
	\item {Thiết kế và triển khai các phiên bản học không giám sát trực tuyến}\index{học không giám sát trực tuyến (Online unsupervised learning)} nhằm cập nhật trọng tâm và ma trận phân vùng dựa trên các lô dữ liệu nhỏ\index{lô dữ liệu nhỏ (mini-batch)} hoặc từng hàm mật độ khi chúng đến liên tục. Nhờ vậy, có thể giảm đáng kể yêu cầu về bộ nhớ và thời gian tính toán, đồng thời nâng cao khả năng xử lý trong môi trường dữ liệu động.
\end{enumerate}
...
\begin{enumerate}[label=(\roman*), leftmargin=2em, itemsep=1ex]	
	\item Phát triển các kỹ thuật trích xuất đặc trưng tiên tiến hơn để biểu diễn các đặc tính hình ảnh phức tạp như kết cấu\index{đặc trưng kết cấu (texture feature)} và hình dạng\index{đặc trưng hình dạng (shape feature)} thành hàm mật độ xác suất. Đồng thời, nên mở rộng việc trích xuất đặc trưng mật độ để xử lý dữ liệu ảnh đa kênh màu.
	\item Xây dựng cơ chế xác định siêu tham số tối ưu cho các thuật toán phân chùm hàm mật độ. Song song đó, phát triển các công cụ giải thích-AI\index{giải thích AI (XAI)} chuyên biệt cho các mô hình nhận dạng thống kê hàm mật độ nhằm tăng tính minh bạch và khả năng diễn giải, giúp người dùng hiểu rõ lý do mô hình phân loại một mẫu vào lớp thiểu số.
	\item Mở rộng việc áp dụng nhận dạng thống kê cho dữ liệu hàm mật độ đa chiều vốn xuất hiện ngày càng nhiều trong các nghiên cứu về mối tương quan phức tạp giữa các biến.
\end{enumerate}
...
\paragraph{Bài giải Ví dụ~\ref{VD:thubac}.} Đầu tiên, ta xem thứ tự của $\mu_i$ chính là thứ tự của dữ liệu hàm mật độ $f_i$, tính toán các khoảng cách từng đôi hàm mật độ bất kỳ bằng khoảng cách $\mathcal{L}_2(f_i\,;f_j)$ với $i\,;j = 1\,;2\,;\ldots\,;7$, ta nhận được ma trận khoảng cách
$$
\bm D^{(t=0)} =
\begin{array}{c|ccccccc}
...
\begin{table}[H]
	...
	\begin{tabular*}{\textwidth}{@{\extracolsep\fill}p{1.5cm}ccccccccp{2cm}@{\extracolsep\fill}}
		\toprule
...
\chapter*{PHỤ LỤC C\\BÀN LUẬN}\label{phulucA}


\section*{1.\quad Về phương pháp trích xuất hình ảnh}
...
\section*{2.\quad Về thuật toán phân tích chùm các hàm mật độ xác suất}
...
\section*{3.\quad Về thuật toán phân tích chùm các hàm mật độ xác suất}

Thuật toán IFCF cải tiến hàm mục tiêu giúp trung hòa đóng góp của các chùm lớn hơn, từ đó giảm "hiệu ứng đồng nhất". Tuy nhiên, trong quá trình lặp, giả sử hai trọng tâm $\Theta$ được xác định, khi đó với bất kỳ hàm mật độ $j$ thuộc chùm $c_1$, các mức độ phân vùng được cập nhật theo Công thức~\eqref{eq:ifcf_u_final}, và điều kiện $\sum_{j=1}^c u_{ij}=1$, có thể được viết lại thành
\begin{align*}
...
Khi $\omega_1 \to \infty$, ta có $
	\lim_{\omega_1 \to \infty} \mu_{i1} = 1$,
	$\lim_{\omega_1 \to \infty} \mu_{i2} = 0.$ Điều này cho thấy khi tỷ lệ mất cân bằng ($\text{IR}$) tăng đến vô cùng, mức độ phân vùng mờ cho chùm lớn sẽ tiến về 1, và chùm nhỏ sẽ tiến về 0. Hiện tượng này được gọi là phân cực trong các mức độ phân vùng. Ta vẫn chưa có cơ chế phát triển nào cho hiên tượng này mặt dù đã có cải tiến.





\section*{4.\quad Về thuật toán phân loại các hàm mật độ xác suất}
...
\section*{5.\quad Về các chỉ số đánh giá nhận dạng hàm mật độ xác suất}
