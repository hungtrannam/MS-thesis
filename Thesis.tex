...
\begin{table}[H]
	...
	\begin{tabular*}{\columnwidth}{@{\extracolsep\fill}ll@{\extracolsep\fill}}
		...
		$ \mathcal{F}\,,\mathcal{G}\,, \mathcal{H}\,, \mathcal{P} $   & Tập hợp                                                     \\
		...
		$f*g$                                                         & Toán tử tích chập giữa hai hàm mật độ $f$ và $g$            \\
		...
		$\langle \bm A\,,\bm B\rangle$ & Tích trong của hai ma trận (bằng $A^\intercal B$) \\\bottomrule
	\end{tabular*}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular*}{\columnwidth}{@{\extracolsep\fill}lp{6cm}p{6cm}@{\extracolsep\fill}} 
		...
		XAI & Trí tuệ nhân tạo giải thích được & e\textbf{X}plainable \textbf{A}rtificial \textbf{I}ntelligence \\
		\bottomrule
	\end{tabular*}
\end{table}
...
Hàm mật độ xác suất là khách thể nghiên cứu của các nhà thống kê từ những năm 2010 bởi Tai Vo-Van \cite{Tai2010}. Ở đây, luận văn chỉ hệ thống hóa những kiến thức có liên quan.
...
\chy{
Các giá trị của mật độ xác suất thì có thể tùy ý, nhưng phải thỏa mãn Tính chất \ref{tc1}. Một xác suất tại điểm $x=x_0$ không được khảo sát trong hàm mật độ liên tục tuyệt đối mà ở một tập Borel thực như Tính chất \ref{tc1}.(3). Tức là xác suất tại điểm bất kì luôn bằng $0$ đối với biến ngẫu nhiên $X$ liên tục.
}
...
Để minh họa và mô phỏng, luận văn này làm việc trên dữ liệu các hàm mật độ định nghĩa trên toàn không gian mẫu $I=\mathbb{R}$ hoặc $\bm R=\mathbb{R}^d$, và ký hiệu xuyên suốt là $\mathcal{F}=\{f_1(x)\,,\ldots\,,f_n(x)\}$. Để đơn giản hóa, chúng tôi giả định các đối tượng hàm mật độ liên tục tuyệt đối trên tập số thực là độc lập và cùng phân phối.

\chy{Dữ liệu hàm mật độ xác suất có cấu trúc phức tạp và tốn nhiều thời gian thu thập. Hầu hết các dữ liệu được nghiên cứu đều là dữ liệu mô phỏng chùm hàm mật độ như \cite{nguyen2023globally,vo2024supervised}. Trong thực tế, dữ liệu hàm mật độ $1$-chiều có thể khám phá cơ cấu phân bố loài, đơn cử là dữ liệu Cá hồi vây vàng (\textit{Yellowfin tuna}) của \cite{minami2024regression} biểu diễn phân bố chiều dài của cá qua các ô tọa độ vùng biển Thái Bình Dương. Ngoài ra, dữ liệu hàm mật độ $2$-chiều thường gặp khi ứng dụng trong phân tích tương quan điểm số của sinh viên các khoa/viện thuộc trường Đại học Cần Thơ. Đặc biệt, dữ liệu hàm mật độ còn được thành lập dựa trên dữ liệu hình ảnh được trích xuất màu sắc nhưng vẫn rất cần được nghiên cứu sâu.
}
...
$$\theta_{G_i}(x) = \arg\min_{f({\bm\mu}\,,\Sigma)}\sum_{j=1}^n\mu_{ij}D(f({\bm\mu}\,,\Sigma)\,,f_j(x)).$$
 ...
 Khác với \eqref{daidien}, hàm mật độ này chỉ cần tìm một hàm Gauss duy nhất mà không cần tổ hợp lồi các $f_j$. Điều này giúp giảm nhiễu khi chùm chứa các hàm có phân tán bất thường, và tăng khả năng khái quát hóa. Hình \ref{fig:hamdaidien} minh họa cụ thể hai cách biểu diễn hàm mật độ đại diện trong cùng một chùm hàm Gauss. Dễ thấy sự khác biệt, hàm mật độ đại diện tập trung vào phân bổ trung bình của nó tại các chùm, còn hàm mật độ đại diện Gauss lựa chọn vị trí dựa vào khoảng cách các hàm mật độ với nhau. 
...
Theo Định nghĩa \ref{D}, khoảng cách giữa hai hàm mật độ thỏa mãn các tiên đề của mê-tríc. Trong lý thuyết thống kê và thông tin, một mê-tríc được gọi là \emph{khoảng cách} và nửa mê-tríc gọi là \emph{phân kỳ}. Ta cũng có mối quan hệ giữa hai hàm mật độ đối mới trọng tâm bất kỳ trong mê-tríc.
...
\begin{table}[H]
	...
	\begin{tabular*}{\columnwidth}{@{\extracolsep\fill}lllccc@{\extracolsep\fill}} 
		...
		Kullback–Leibler & $\int_{\Omega} f(x)\log\frac{f(x)}{g(x)}\mathrm{\,d}x$ & $[0,\infty)$ & & & $\checkmark$ \\
		\bottomrule
	\end{tabular*}
...
\chy{Nhìn vào Bảng \ref{tab1:KC_PDF}, có thể thấy khoảng cách Chernoff là một tổng quát hóa của khoảng cách Bhattacharyya khi chọn $s = 1/2$. Đáng chú ý, cả hai loại khoảng cách $\mathcal{L}_p$ và Hellinger đều thỏa mãn đầy đủ các tính chất mê-tríc, còn Kullback–Leibler  không đối xứng và vi phạm bất đẳng thức tam giác.}
...
Hai hạt nhân cơ bản cho khoảng cách hai hàm mật độ $f_1(x)$ và $f_2(x)$ là hạt nhân chuẩn\index{hàm hạt nhân chuẩn (Gaussian kernel)}
	$
	K_G(f_1, f_2) = \exp\left(-\frac{D^2(f_1\,,f_2)}{\beta\sigma^2}\right),
	$
	trong đó, $D$ thuộc họ khoảng cách $\mathcal{L}_p$ để đảm bảo tính mê-tríc và $\beta\in\mathbb{R}_{>0}$, và hạt nhân Bhattacharyya\index{hàm hạt nhân Bhattacharyya (Bhattacharyya kernel)}
	$
...
\dng{[Ma trận khoảng cách từng đôi]
	...
	\bm D_{ij} = D(f_i, f_j)$, với mọi $i,j \in \{1,\dots,n\}$.
}
...
\chy{Trong Bảng \ref{tab:chumham} Affinity Toussaint suy biến thành Affinity Matusita khi $\alpha_1=\alpha_2=\cdots=\alpha_n=\frac{1}{n}$. Khi Affinity của Matusita có $n=2$ thì nó trở thành hệ số Bhattacharyya \cite{}. Cuối cùng, hệ số tượng tự chùm chuẩn hóa tuyến tính độ rộng chùm.}

\section{Phương pháp số xấp xỉ tích phân}

Các tích phân trong lĩnh vực hàm mật độ cần xấp xỉ bằng phương pháp số. Luận văn này, chúng tôi chọn sử dụng phương pháp hình thang\index{phương pháp hình thang (trapezoidal method)} nhằm cải thiện độ chính xác so với tổng Riemann nhưng cũng không tốn quá nhiều tài nguyên như Monte Carlo hay Simpsons. Một tích phân trên miền $\Omega \subset \mathbb{R}^d$ được xấp xỉ bằng công thức tổng quát
...
Thuật toán phân chùm dựa vào trọng tâm đề xuất hàm mục tiêu khác nhau, nhưng mục tiêu chung là sự phân bổ tối tiểu khoảng cách phần tử trong chùm trong khi vẫn giữ tối đa khoảng cách phần tử giữa các chùm. Các thuật toán này được chia thành hai loại dựa theo lý thuyết mờ \cite{bezdek2013pattern} là phân chùm không mờ (bao gồm $k$-mean \cite{nguyen2024new}, phân cấp thứ bậc\index{phân cấp thứ bậc ()} và phân chùm tự cập nhật\index{phân chùm tự cập nhật (self-updating process)}) và phân chùm mờ (bao gồm Fuzzy $c$-means \cite{nguyentrang2017fuzzy,vovan2019cluster}).
...
\chm{
Điều kiện \(\sum_{i} \mu_{ij} = 1\) đảm bảo rằng mỗi hàm \(f_j\) được gán vào đúng một cụm duy nhất.
Tập tất cả các ma trận \(U\) khả dĩ là hữu hạn (với kích thước \(k^n\)), và việc cố định \(U\) biến hàm mục tiêu~\ref{Jk} trở thành hàm tuyến tính theo \(U\).
Đặc biệt, với \(\theta_i\) cố định, giá trị \(J\) được tối thiểu khi gán \(f_j\) vào cụm \(c_i\) sao cho \(D^2(f_j, \theta_i)\) nhỏ nhất, tức là
$
...
$. Hơn nữa, vì ma trận phân vùng  $\mu_{ij}= \bm 1_{\{j\in c_i\}}$ nên ta có thể rút gọn hàm mật độ đại diện \eqref{daidien} thành trung bình Fréchet
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.8\linewidth]{Figs/C1_phanvung.pdf}
	\begin{flushleft}
		\footnotesize \it Chú thích: Giá trị $\mu_{1j}$ tại trục tung biểu diễn mức độ phân vùng tại chùm 1 theo khoảng cách $\bm D_{1j} = \mathcal{L}_1$ với các giá trị $m=1{,}1; 1{,}5; 2{,}0; 3{,}0; 5{,}0; 10{,}0$. Khi $m\to1$, đường phân vùng trở nên gãy; khi $m$ tăng, đường cong trở nên phẳng và đều ở $y=1/2$.
...
\vdu{[Phân chùm mờ hàm Gauss $1$-chiều]\label{VD:FCF} Ta sử dụng lại dãy hàm mật độ trong Ví dụ~\ref{VD:thubac}. Phân tích $\mathcal{F}$ thành ba chùm, biết $m=2$ và khoảng cách là $\mathcal{L}_2$. Ta có kết quả sau khi thực hiện các vòng lặp được giải chi tiết ở Phụ lục (tr. \pageref{phulucB}).
...
$$ 
trong đó, $\pi _j = {\mathbb {P}}(Y_j = 1 \vert f(x))$ và $\mathcal{L}$ là hàm mất mát\index{hàm mất mát (loss function)}.

\begin{theorem}\label{dly:loi}
	Hàm mất mát $\mathcal{L}$ là một hàm lồi theo cả $\beta_0$ và $\beta(x)$. Nếu $\lambda_2 > 0$ thì hàm này lồi nghiêm ngặt.
\end{theorem}

Từ Định lý~\ref{dly:loi}, ta tính được Gradient theo hệ số $\beta(x)$ với $\hat{y}_j = \pi_j$ là xác suất dự đoán, có dạng
	$
	\nabla_{\bm\beta} \mathcal{L} = \Phi^\intercal (y - \hat{y}) - \lambda_1 \,\text{sign}(\beta(x)) - \lambda_2 \beta,
	$
...
\dly{Bài toán đối ngẫu \ref{eq:svm-dual} là bài toán tối ưu lồi nghiêm ngặt trên đa diện lồi compact, do đó tồn tại duy nhất nghiệm véc-tơ \(\boldsymbol{\alpha}^{*}\in\mathbb{R}^{n}\).}
...
\dly{Tại nghiệm $\bm{\alpha}^*$, khoảng cách đến siêu phẳng tối ưu là
	$$
	\rho =\frac{1}{\lVert\bm w\rVert} =\left( \sum_{j=1}^n \sum_{k=1}^n \alpha_j^\star \alpha_k^\star y_j y_k K(f_j, f_k) \right)^{-1/2}.
	$$
	Khi đó, hàm phân lớp tương ứng đối với hàm mật độ mới $h$ là
	\[
	\hat{y}(h) = \operatorname{sign}\!\left( g(h) \right) = 
	\operatorname{sign}\!\left( \sum_{j=1}^n \alpha_j^\star y_j K(f_j, h) + b^\star \right).
	\]
}
...
\vdu{[Phân loại hàm mật độ bằng phương pháp Bayes] Trở lại giả thuyết của Ví dụ~\ref{VD:log}, ta cần phân loại hàm mật độ Gauss mới $h(x)=\mathcal{N}(3,0;0,9)$ vào một trong hai lớp.
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.5\linewidth]{Figs/C1_NB}
		\caption{}
		\label{fig:c1nb}
	\end{figure}
...
Trong bài toán phân loại Bayes, xác suất tiên nghiệm $\mathbb{P}(c_k)$ thường được ước lượng bằng nhiều cách khác nhau, thông thường là dựa vào kiến thức chuyên gia hoặc thông tin trong lịch sử nghiên cứu tương tự hoặc dữ liệu lớn. Trong trường hợp không có thông tin tiên nghiệm nào, ta có thể giả định mọi lớp đều có xác suất bằng nhau, tức là $\mathbb{P}(c_k)=1/k$. Trong nhiều tình huống, xác suất tiên nghiệm mà còn có thể ước lượng từ tần suất xuất hiện của các lớp trong tập dữ liệu huấn luyện. Khi đó, $\mathbb{P}(c_k)=n_k/n$  trong đó $n_k$là số lượng mẫu thuộc lớp $c_k$ và $n$ là tổng số mẫu. Tuy nhiên, phương pháp này có thể gặp vấn đề khi dữ liệu ít hoặc có xác suất bằng 0 cho một số lớp, phương pháp Laplace được sử dụng để điều chỉnh, $\mathbb{P}(c_k) = (n_k+\alpha)/(n+\alpha k)$, trong đó $\alpha>0$ và $k$ là số lớp.
...
\begin{array}{c|cc}
	...
	\midrule
	\text{Thực tế dương tính} & \text{Dương tính thật } (\#TP) & \text{Âm tính thật } (\#FN) \\
	\text{Thực tế âm tính} & \text{Dương tính giả } (\#FP) & \text{Âm tính giả } (\#TN) \\
	\bottomrule
\end{array}
\]
}
